...
-- [ML].[MLPatternValidation]  10337 ,'10/22/2018','1/21/2019','627384' 
-- =============================================  

CREATE PROCEDURE [ML].[SaveUploadPatternValidation]    
(    
@UserId NVARCHAR(10)=null,    
@ProjectID NVARCHAR(200)=null,    
@lstApprovedPatternValidation ML.TVP_SaveApprovedUploadPatternValidation READONLY    
)    
AS     
BEGIN    
BEGIN TRY    
BEGIN TRAN    
 ...
 case when isnull(ResolutionRemarksSubPattern,'0')='' THEN '0' else ResolutionRemarksSubPattern END as ResolutionRemarksSubPattern  
 FROM @lstApprovedPatternValidation PV    
 ...
 JOIN [AVL].[DEBT_MAP_CauseCode] CC    
  ...
  AND RC.IsDeleted =0    
 JOIN AVL.APP_MAS_ApplicationDetails AP    
  ON AP.ApplicationName=PV.Application    
  AND AP.IsActive=1    
 JOIN AVL.APP_MAP_ApplicationProjectMapping AM    
  ON AM.ApplicationID=AP.ApplicationID    
  AND AM.ProjectID = @ProjectID    
  AND AM.IsDeleted = 0    
     
 UPDATE D set D.IsApprovedOrMute = (case when V.IsApprovedOrMute =-1 then D.IsApprovedOrMute else 0 end)  
 FROM ML.TRN_PatternValidation D    
 JOIN #DebtApprovedPVTickets V    
  ON  V.DescriptionBasePattern=D.TicketPattern  and  
  V.DescriptionSubPattern=D.subPattern and  
  V.ResolutionRemarkBasePattern=D.additionalPattern and  
  V.ResolutionRemarksSubPattern=D.additionalSubPattern    
  AND V.ApplicationID=D.ApplicationID      
...
BEGIN CATCH      
  ...
  ROLLBACK TRAN    
  --INSERT Error        
  EXEC AVL_InsertError '[ML].[SaveUploadPatternValidation]]', @ErrorMessage, @ProjectID,0    
