...
CREATE PROCEDURE [ML].[SaveTRN_ClusteringOutcomeUploadedData_App]  
(  
...
IF(@IsRegenerate=0)  
BEGIN  
SELECT distinct CTA.MLTransactionId,CTA.ProjectID,CTA.TicketID,CTA.ApplicationID,CTA.DebtClassificationID,  
CTA.AvoidableFlagID,CTA.ResidualDebtID,CTA.ClusterID_Desc,CTA.ClusterID_Resolution  
into #Temp_Cluster  
FROM ML.TRN_ClusteringTicketValidation_app CTA  
inner join #ClusterOutcome CO ON CTA.ApplicationID=CO.ApplicationID and   
...
SELECT distinct MLTransactionId,ProjectID,TicketID,ApplicationID,DebtClassificationID,  
AvoidableFlagID,ResidualDebtID,ClusterID_Desc,ClusterID_Resolution  
FROM ML.TRN_ClusteringTicketValidation_app WHERE MLTransactionId=@MLTransactionId and ClusterID_Desc=0  
and ClusterID_Resolution=0  
...
DELETE FROM ML.TRN_ClusteringOutcomeUploadedData_App WHERE MLTransactionId = @MLTransactionId  
  
INSERT INTO ML.TRN_ClusteringOutcomeUploadedData_App  
(MLTransactionId,ProjectID,TicketID,ApplicationID,DebtClassificationID,  
AvoidableFlagID,ResidualDebtID,ClusterID_Desc,ClusterID_Resolution,IsActive  
,CreatedBy,CreatedDate)  
SELECT distinct MLTransactionId,ProjectID,CO.TicketID,CO.ApplicationId,DebtClassificationID,  
...
from ML.TRN_ClusteringTicketValidation_app T inner join #ClusterOutcome CO  
ON T.ClusterID_Desc=CO.ClusterID_Desc and T.ClusterID_Resolution=CO.ClusterID_Resolution  
INNER JOIN AVL.DEBT_MAS_DebtClassification DC ON DC.DebtClassificationName=CO.DebtClassification  
INNER JOIN AVL.DEBT_MAS_AvoidableFlag AF ON AF.AvoidableFlagName=CO.AvoidableFlag  
INNER JOIN AVL.DEBT_MAS_ResidualDebt RD ON RD.ResidualDebtName=CO.ResidualDebt  
...
JOIN [ML].TRN_ClusteringTicketValidation_app UD ON     
