...
-- [ML].[Infra_GetIconDetailsByChoice] 8354,2
-- =============================================  
CREATE PROCEDURE [ML].[Infra_GetIconDetailsByChoice] 
  @ID BIGINT, --ProjectId, 
  @Choice INT,
  @IsRegenerate BIT
AS 
  BEGIN 
	  ...
	  SET NOCOUNT ON;
		  ...
		  SET @MlCount = (SELECT COUNT(*) FROM ML.InfraConfigurationProgress (NOLOCK)
										WHERE ProjectID = @ID AND IsDeleted = 0)
		  IF(@IsRegenerate=0 AND @MlCount > 1)
		  BEGIN
				INSERT INTO #InitialLearningDetails
						SELECT ID FROM ML.InfraConfigurationProgress (NOLOCK)
                           				WHERE PROJECTID = @ID 
										AND ISDELETED = 0 
		  ...
		  SELECT @IsTicketDescOpt = IsTicketDescriptionOpted,@DebtAttributes = DebtAttributeId
				FROM ML.InfraConfigurationProgress(NOLOCK)WHERE  PROJECTID = @ID 
													 AND ISDELETED = 0 
                                                     ORDER  BY ID DESC

		  IF(@IsRegenerate=0 AND @MlCount > 1)
		  BEGIN
				SET @StartDate = (SELECT MIN(FromDate) FROM ml.InfraConfigurationProgress(NOLOCK)
														WHERE ProjectID=@ID 
														AND IsDeleted=0
														AND ISNULL(IsMLSentOrReceived,'')='Received')

				SET @EndDate = (SELECT MAX(ToDate) FROM ml.InfraConfigurationProgress(NOLOCK)
												   WHERE ProjectID=@ID 
												   AND IsDeleted=0 
												   AND ISNULL(IsMLSentOrReceived,'')='Received')
		  END
            ...
            BEGIN 
                DECLARE @TicketAnalysed   BIGINT = 0, 
                      ...
                      SET @TicketAnalysed = (SELECT COUNT(DISTINCT TV.TICKETID) 
                                             FROM   ML.InfraTICKETVALIDATION (NOLOCK) TV
                ...
                AND TV.TicketDescriptionBasePattern <> '0' ))) 
                      ...
                      SET @SamplingCount = (SELECT COUNT(DISTINCT TS.TICKETID) 
                                            FROM   ML.InfraTRN_TicketsAfterSampling(NOLOCK) TS
													...
													AND TS.DebtClassifiedBy in (2,4)) 
   ...
   AND IL.ISDELETED = 0 
            ...
            AND TC.PROJECTID = TS.PROJECTID 
                     ...
                     AND TS.ISDELETED = 0 AND TS.DebtClassifiedBy in (2,4)
                                             ) 
          ...
          ON TC.TICKETID = TV.TICKETID 
                ...
                ELSE 
                  BEGIN 
                      SET @TicketConsideredMLTC = (SELECT COUNT(DISTINCT MB.TICKETID) 
                                                   FROM   ML.InfraBaseDetails (NOLOCK) MB
                                                          JOIN #TMPCOUNT TC 
           ...
           AND MB.RESOLUTIONCODE <> '' ) 
               AND ( MB.RESIDUALDEBT IS NOT NULL 
                            ...
                            AND NOT EXISTS (SELECT DISTINCT TS.TicketID 
                                                                          FROM   [ML].[InfraTRN_TicketsAfterSampling] TS
   ...
   FROM   ML.InfraTRN_PATTERNVALIDATION (NOLOCK) 
      ...
      BEGIN CATCH 
          ...
          EXEC AVL_INSERTERROR 
            '[ML].[Infra_GetIconDetailsByChoice]', 
            @ErrorMessage1, 
            @ID, 
            0 
     END CATCH 
