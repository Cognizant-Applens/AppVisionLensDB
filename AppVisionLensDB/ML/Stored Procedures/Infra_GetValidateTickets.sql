...
-- Description:      SP for ML - Infra 
-- Test:             EXEC [ML].[Infra_GetValidateTickets] 10337,2 
-- ============================================================================  
CREATE PROC [ML].[Infra_GetValidateTickets]
 @ID BIGINT,
 @Choose TINYINT,
  ...
  BEGIN 
	  ...
	  SET @LatestID = (CASE 
				  WHEN @IsRegenerate = 0 THEN ( SELECT TOP 1 ID FROM   ML.InfraConfigurationProgress(NOLOCK)
					...
					ELSE ( SELECT max(ID) FROM   ML.InfraConfigurationProgress (NOLOCK)
		...
		IF EXISTS(SELECT TOP 1 * FROM AVL.MAS_ProjectMaster (NOLOCK) WHERE ProjectID= @ID AND IsDeleted = 0 AND IsMultilingualEnabled = 1)
		BEGIN
	   ...
	   SELECT 
		...
		FROM
		ML.InfraTicketValidation (NOLOCK) TV
		JOIN ML.InfraConfigurationProgress  (NOLOCK) CP ON CP.ProjectID = TV.ProjectID AND CP.ID=TV.InitialLearningID
		JOIN AVL.MAS_ProjectMaster(NOLOCK) PM ON PM.ProjectID = TV.ProjectID AND PM.IsDeleted = 0
		JOIN AVL.TK_TRN_InfraTicketDetail(NOLOCK) TD ON TD.TicketID = TV.TicketID AND TD.ProjectID = TV.ProjectID AND TD.IsDeleted = 0
		LEFT JOIN AVL.TK_TRN_Multilingual_TranslatedInfraTicketDetails (NOLOCK) MTT ON MTT.TimeTickerID = TD.TimeTickerID
