...
-- [ML].[SaveSamplingDetailsFromGrid]
-- =============================================  
CREATE PROCEDURE [ML].[SaveSamplingDetailsFromGrid] 
					(@UserId                 NVARCHAR(50), 
...
AS 
  BEGIN 
      BEGIN TRY 
		  ...
		  SET @InitiId = (SELECT TOP 1 ID FROM ML.ConfigurationProgress
          ...
          FROM   ML.TRN_TicketsAfterSampling TAS 
                 JOIN #TMPTICKET TT 
                   ON TT.TicketID = TAS.TicketID 
					  ...
					  WHERE TAS.InitialLearningId = @InitiId
