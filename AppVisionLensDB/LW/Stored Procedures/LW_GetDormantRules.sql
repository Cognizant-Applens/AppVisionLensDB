...
CREATE PROCEDURE [LW].[LW_GetDormantRules]       
@flag VARCHAR(20),      
...
SELECT rule_transaction.[RecordID],  
    ...
    DATEDIFF(day,rule_transaction.[CreatedDate], GETDATE()) AS RuleAge,        
       ...
       rule_transaction.[RuleLevel] = @RuleLevel  AND rule_usage.[IsDormant] = 1;        
 END      
...
SELECT rule_transaction.[RecordID],      
    ...
    rule_transaction.RuleReferanceID AS NewRuleReferenceID,        
       ...
       ON rule_transaction.[RuleID] = master_rules.[RuleID]         
       WHERE rule_transaction.[AccountID] is null        
       AND rule_transaction.[BUID] = @BUID AND        
       rule_transaction.[RuleLevel] = @RuleLevel  AND rule_transaction.[IsDormant] = 1 and rule_transaction.IsDeleted=0 and master_rules.IsDeleted=0;        
...
DECLARE @ErrorMessage VARCHAR(MAX);        
              ...
              --INSERT Error            
              EXEC AVL_InsertError  '[LW].[LW_GetDormantRules]'        
       ,@ErrorMessage        
       ,''        
       ,0        
END CATCH        
