...
CREATE PROCEDURE [ADM].[ALM_GetBacklogAccessByExecutionMethod]
(
@userId VARCHAR(50),
@projectId BIGINT
)
 ...
 BEGIN CATCH
  DECLARE @ErrorMessage VARCHAR(MAX);
	SELECT @ErrorMessage = ERROR_MESSAGE()	
		EXEC AVL_InsertError '[ADM].[ALM_GetBacklogAccessByExecutionMethod] ', @ErrorMessage, @userId,@projectId
		RETURN @ErrorMessage
  END CATCH             
END
