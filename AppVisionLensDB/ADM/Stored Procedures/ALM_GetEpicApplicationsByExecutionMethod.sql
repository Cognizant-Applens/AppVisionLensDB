...
CREATE PROCEDURE [ADM].[ALM_GetEpicApplicationsByExecutionMethod]
(
@projectId BIGINT
)
AS            
...
FROM #tempProjectLevelExecutionMethod(NOLOCK) AS PLE
 ...
 BEGIN CATCH
  DECLARE @ErrorMessage VARCHAR(MAX);
	SELECT @ErrorMessage = ERROR_MESSAGE()	
		EXEC AVL_InsertError '[ADM].[ALM_GetEpicApplicationsByExecutionMethod] ', @ErrorMessage, 0,@projectId
		RETURN @ErrorMessage
  END CATCH             
END
