/***************************************************************************
*COGNIZANT CONFIDENTIAL AND/OR TRADE SECRET
*Copyright [2018] – [2021] Cognizant. All rights reserved.
*NOTICE: This unpublished material is proprietary to Cognizant and
*its suppliers, if any. The methods, techniques and technical
  concepts herein are considered Cognizant confidential and/or trade secret information. 
  
*This material may be covered by U.S. and/or foreign patents or patent applications. 
*Use, distribution or copying, in whole or in part, is forbidden, except by express written permission of Cognizant.
***************************************************************************/


CREATE PROCEDURE [ADM].[ALM_GetEstimationAndEffortByExecutionMethod]
(
@userId VARCHAR(50),
@projectId BIGINT,
@applicationId BIGINT,
@workItemTypeId  BIGINT
)
AS            
BEGIN     
BEGIN TRY       
SET NOCOUNT ON;  

DECLARE @executionId BIGINT


SELECT @executionId=ExecutionMethod 
FROM [ADM].[ALMApplicationDetails](NOLOCK) WHERE  ApplicationID=@applicationId


SELECT 
IsEstimationPoints AS 'IsEstimationMandate',
IsEffortTracking AS 'IsEffortTrackingMandate'
FROM [PP].[ALM_MAP_GenericWorkItemConfig](NOLOCK) 
WHERE projectid= @projectId AND ExecutionId=@executionId 
AND WorkItemTypeId=@workItemTypeId AND IsDeleted=0



SET NOCOUNT OFF;
END TRY 
 BEGIN CATCH
  DECLARE @ErrorMessage VARCHAR(MAX);
	SELECT @ErrorMessage = ERROR_MESSAGE()	
		EXEC AVL_InsertError '[ADM].[ALM_GetEstimationAndEffortByExecutionMethod] ', @ErrorMessage, @userId,@projectId
		RETURN @ErrorMessage
  END CATCH             
END
