...
CREATE PROCEDURE [AVL].[InfraGetSamplingDetailsForML] 
...
SELECT TOP 1 @StartDate=StartDate,@EndDate=EndDate from AVL.ML_PRJ_InitialLearningStateInfra(NOLOCK) WHERE ProjectID=@ProjectID AND IsDeleted=0 ORDER BY ID DESC
...
FROM avl.ML_TRN_TicketsAfterSamplingInfra(nolock) A
JOIN #tmpml(nolock) TD
	ON A.Projectid = @ProjectID
	AND A.Initiallearningid = @LatestID
	AND TD.Ticketid = A.Ticketid
	AND A.Isdeleted = 0
LEFT JOIN [AVL].[DEBT_MAS_DebtClassificationInfra](nolock)
AM
...
LEFT JOIN [AVL].InfraTowerDetailsTransaction(NOLOCK) ITDT
	ON A.TowerID = ITDT.InfraTowerTransactionID 
...
LEFT JOIN [AVL].[BUSINESSUNIT] DM
	ON DAM.Buid = DM.Buid
	AND DM.Isdeleted = 0
INNER JOIN avl.ML_TRN_RegeneratedTowerDetails
rg
	ON rg.Projectid = A.Projectid
	AND rg.Isdeleted = 0
	AND rg.Initiallearningid = @LatestID
	AND rg.TowerID = A.TowerID
WHERE A.Projectid = @ProjectID
AND A.Isdeleted = 0
...
LEFT JOIN [AVL].[BUSINESSUNIT](nolock) DM
	ON DAM.Buid = DM.Buid
	AND DM.Isdeleted = 0
INNER JOIN avl.ML_TRN_RegeneratedTowerDetails(nolock)
rg
	ON rg.Projectid = TV.Projectid
	AND rg.Isdeleted = 0
	AND rg.Initiallearningid = @LatestID
	AND rg.TowerID = TV.TowerID
WHERE TV.Projectid = @ProjectID
AND TV.Isdeleted = 0
AND NOT EXISTS (SELECT
		Ticketid
...
--INSERT Error       
EXEC AVL_INSERTERROR	'[AVL].[InfraGetSamplingDetailsForML]'
						,@ErrorMessage
						,@ProjectID
						,0
END CATCH
