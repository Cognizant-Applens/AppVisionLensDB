...
CREATE PROCEDURE [AVL].[InfraFinalTicketDetailsforcallingML] 
...
SET @InitID = (SELECT TOP 1
	...
	WHERE ProjectID = @ProjectID
	ORDER BY ID DESC)
SET @OptionalFieldID = ISNULL((SELECT TOP 1
		OptionalFieldID
...
SELECT
	DISTINCT DM.BUName AS DepartmentName
...
OR (@IsRegeneratedML = 0))--if it transaction id is regenertaed or not ,if yes then that tower details will only be selected
...
--INSERT Error     
EXEC AVL_INSERTERROR	'[AVL].[InfraFinalTicketDetailsforcallingML]'
						,@ErrorMessage
						,@ProjectID
						,0
END CATCH
