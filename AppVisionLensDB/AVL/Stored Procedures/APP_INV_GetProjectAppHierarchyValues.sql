...
CREATE PROCEDURE [AVL].[APP_INV_GetProjectAppHierarchyValues] 
@CustomerID bigint,
@ProjectID bigint
AS
BEGIN
...
DECLARE @sqlCommand varchar(1000);	
	...
	SELECT 
		...
		,CASE WHEN EXISTS (SELECT 1 FROM avl.TK_TRN_TicketDetail TN JOIN 
						AVL.APP_MAP_ApplicationProjectMapping MP 
						ON MP.ApplicationID=TN.ApplicationID WHERE TN.ApplicationID = AD.ApplicationID
						AND MP.ProjectID=@ProjectID  AND TN.IsDeleted=0 AND MP.IsDeleted=0)
       THEN 1 
       ELSE 0
		END AS 'HasTickets'  
	...
	SELECT 
		Checked,HasTickets,AL.ApplicationID,AL.ApplicationName,AL.BusinessClusterBaseName, AL.BusinessClusterMapID,AL.BusinessClusterID,
	...
	IF @count >0 
		BEGIN
				...
				BEGIN
					...
					IF @count >0 
						BEGIN
							...
							SELECT 
								Checked,HasTickets,AL.ApplicationID,AL.ApplicationName,AL.BusinessClusterBaseName5,AL.BusinessClusterMapID5,
					...
					ELSE
						BEGIN
							SELECT 
								Checked,HasTickets,AL.ApplicationID,AL.ApplicationName,AL.BusinessClusterBaseName4,AL.BusinessClusterMapID4,
								AL.BusinessClusterBaseName3,AL.BusinessClusterMapID3,
								AL.BusinessClusterBaseName2,AL.BusinessClusterMapID2,
								AL.BusinessClusterBaseName1,AL.BusinessClusterMapID1,
								AL.BusinessClusterBaseName,AL.BusinessClusterMapID
			...
			ELSE
				BEGIN
					SELECT 
						Checked,HasTickets,AL.ApplicationID,AL.ApplicationName,AL.BusinessClusterBaseName3,AL.BusinessClusterMapID3,
						AL.BusinessClusterBaseName2,AL.BusinessClusterMapID2,
						AL.BusinessClusterBaseName1,AL.BusinessClusterMapID1,
						AL.BusinessClusterBaseName,AL.BusinessClusterMapID
					FROM 
	...
	ELSE
		BEGIN
			SELECT 
				Checked,HasTickets,AL.ApplicationID,AL.ApplicationName,AL.BusinessClusterBaseName2,AL.BusinessClusterMapID2,
				AL.BusinessClusterBaseName1,AL.BusinessClusterMapID1,
				AL.BusinessClusterBaseName,AL.BusinessClusterMapID 
			FROM 
				#App_level3 AL;
...
BEGIN CATCH  
		...
		EXEC AVL_InsertError ' [AVL].[APP_INV_GetProjectAppHierarchyValues]', @ErrorMessage, 0, @CustomerID 
