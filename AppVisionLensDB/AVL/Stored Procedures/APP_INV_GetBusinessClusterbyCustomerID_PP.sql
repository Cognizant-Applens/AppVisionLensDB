...
Create PROCEDURE [AVL].[APP_INV_GetBusinessClusterbyCustomerID_PP]    
 @CustomerID BIGINT                  
AS                  
BEGIN                  
BEGIN TRY                  
...
DECLARE @sqlCommand varchar(1000);             
 ...
 WHERE            
  CustomerID = @CustomerID AND IsDeleted=0   order  by BusinessClusterID                 
 SELECT                   
 ...
 ELSE        
  BEGIN        
   SELECT       
     AL.BusinessClusterMapID1,    
     AL.BusinessClusterBaseName2 AS LOB,    
     AL.BusinessClusterBaseName1 AS Portfolio,    
    AL.BusinessClusterBaseName  AS AppGroup    
   FROM         
    #App_level3 AL ORDER BY AL.BusinessClusterID ASC;        
  END        
...
BEGIN CATCH                    
  ...
  EXEC AVL_InsertError ' [AVL].[APP_INV_GetBusinessClusterbyCustomerID_New]', @ErrorMessage, 0, @CustomerID                                 
 END CATCH                   
END
