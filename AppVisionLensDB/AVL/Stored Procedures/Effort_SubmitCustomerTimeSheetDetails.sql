...
--drop PROCEDURE [AVL].[Effort_SubmitCustomerTimeSheetDetails]
CREATE PROCEDURE [AVL].[Effort_SubmitCustomerTimeSheetDetails]
@EmployeeID nvarchar(1000),
@Status nvarchar(50),
@TVP_TicketDetailsCollection TVP_CustomerEffortTimesheetDetailsSubmit READONLY	                
...
SET NOCOUNT ON;  
	...
	[IsAttributeUpdated] NVARCHAR(max) NULL	
	)
...
BEGIN CATCH  
		...
		ROLLBACK TRAN
		--INSERT Error    
		EXEC AVL_InsertError '[AVL].[Effort_SubmitCustomerTimeSheetDetails] ', @ErrorMessage, @EmployeeID,0
