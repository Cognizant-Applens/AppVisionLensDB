...
CREATE PROCEDURE [AVL].[GetAccountlevelTicketDetailsWithService]
@UserId VARCHAR(20)
...
select distinct TD.TicketID,TMD.TimeTickerID,TD.CreatedBy,TT.TicketTypeName,TD.EffortTillDate, TMD.Hours ,TD.OpenDateTime,TD.Closeddate,TD.DARTStatusID,DS.DARTStatusName,
                TMD.ServiceID,MS.ServiceName,TMD.ActivityID,SAM.ActivityName,AD.ApplicationId,AD.ApplicationName,VP.CustomerId,VP.ESA_AccountID
			  ...
			  and td.OpenDateTime> DATEADD(year,-1,GETDATE()) and DS.DARTStatusID not in(5,7) and TT.supporttypeid=3
...
BEGIN CATCH  
		...
		--INSERT Error    
		EXEC AVL_InsertError '[AVL].[GetAccountlevelTicketDetailsWithService]',@ErrorMessage,@UserId,0
