...
-- Description:   save ticket session detaisl
-- AppVisionLens - App Lens DB, [AVMDART] - AVM DART DB
--   EXEC  [AVL].[Mini_SaveTicketSessionDetails] '471742','UPDATE',25753,'47174215Nov201811','','11/15/2018 5:01:17 AM',null,false,55,754,null,0,false,null,true,0,null,60,null,0,3,111,10115,845,null,false,'11/15/2018 3:31:29 PM'

-- ============================================================================ 

-- EXEC [AVL].[Mini_SaveTicketSessionDetails] '471742','UPDATE',25753,'47174215Nov201811','','11/15/2018 5:01:17 AM',null,false,55,754,null,0,false,null,true,0,null,60,null,0,3,111,10115,845,null,false,'11/15/2018 3:31:29 PM'
CREATE PROCEDURE [AVL].[Mini_SaveTicketSessionDetails]
(
...
BEGIN
	...
	ELSE
		BEGIN
			IF(@TicketID<>'')
				BEGIN
					INSERT INTO AVL.TK_Mini_Sessions(EmployeeID,ProjectID,UserID,TicketID,TicketDesc, TicketOpenDate,StartTime,EndTime,IsDeleted, CreatedOn,CreatedBy,NonTicketDescription,IsRunning,UserCreatedTimeDate)
					SELECT @EmployeeID,@ProjectID,@UserID,@TicketID,@TicketDesc, @TicketOpenDate,ISNULL(@StartTime,GETDATE()),NULL,0,GETDATE(),@EmployeeID,@NonTicketDescription,@Isrunning,@UserDateTime
			...
			ELSE
				BEGIN
					INSERT INTO AVL.TK_Mini_Sessions(EmployeeID,ProjectID,UserID,TicketOpenDate,StartTime,EndTime,IsDeleted, CreatedOn,CreatedBy,IsNonDelivery,NonDeliveryActivityType,NonTicketDescription,IsRunning)
					SELECT @EmployeeID,@ProjectID,@UserID, @TicketOpenDate,ISNULL(@StartTime,GETDATE()),NULL,0,GETDATE(),@EmployeeID,@IsNonDelivery,@NonDeliveryActivityType,@NonTicketDescription,@Isrunning 
...
BEGIN
			...
			(UserID,ProjectID,TicketID,TicketDesc,TicketOpenDate,ApplicationID,ServiceID,ActivityID,TicketTypeMapID
			,PriorityMapID,TicketStatusMapID,StartTime,EndTime,IsAuto,Hours,Minutes,Seconds,IsProcessed,EmployeeID,RequestSource
			,IsSDTicket,IsNonDelivery,NonDeliveryActivityType,IsDeleted,CreatedOn,CreatedBy,ModifiedOn,ModifiedBy,TimeTickerID,IsRunning
			,NonTicketDescription,UserCreatedTimeDate)
			SELECT UserID,
				ProjectID,
				TicketID,
				TicketDesc,
	...
	SELECT SessionID,
				...
				TimeTickerID,
				IsRunning,
				NonTicketDescription FROM AVL.TK_Mini_Sessions WHERE SessionID = @Curent_sessionID
	END
...
ELSE IF (UPPER(@Mode)='NONTICKETACTIVITY')
		...
		ELSE
			BEGIN
				INSERT INTO AVL.TK_Mini_Sessions(EmployeeID,ProjectID,UserID,TicketID,TicketOpenDate,StartTime,EndTime,IsDeleted, CreatedOn,CreatedBy,IsRunning,IsNonDelivery,NonDeliveryActivityType,Hours,Minutes,NonTicketDescription,UserCreatedTimeDate)
				SELECT @EmployeeID,@ProjectID,@UserID,@TicketID, GETDATE(),ISNULL(@StartTime,GETDATE()),NULL,0,GETDATE(),@EmployeeID,1,@IsNonDelivery,@NonDeliveryActivityType,@Hours,@Minutes,@NonTicketDescription,@UserDateTime
...
--TZM1.TZoneName AS ProjectTimeZone FROM #MAS_LoginMaster LM
...
BEGIN CATCH  
		...
		ROLLBACK TRAN
		-- INSERT Error    
		EXEC AVL_InsertError '[AVL].[Mini_SaveTicketSessionDetails]', @ErrorMessage, @UserID,0
