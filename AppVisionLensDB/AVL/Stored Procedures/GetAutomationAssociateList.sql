...
-- Create date: <>
-- Description:	<>
-- Execution  : [AVL].[GetAutomationAssociateList] '',''
-- =============================================
CREATE PROCEDURE [AVL].[GetAutomationAssociateList]
...
BEGIN
	BEGIN TRY
	 SET @FromDate = @FromDate + ' 00:00:00.00'  
      SET @ToDate =  @ToDate + ' 23:59:59:000' 
		select 
		'Automation Solution' AS Category
		,'The Automation Master' AS Award
		,TTT.ProjectID
		,ML.EmployeeID
		,ML.EmployeeName
		,MP.EsaProjectID
		...
		,count(TTT.TicketID) as NoOfATicketsClosed
		...
		and TMT.AVMTicketType=9 group by TTT.AssignedTo,TTT.ProjectID
		,ML.EmployeeID
		,ML.EmployeeName
		,MP.EsaProjectID
		,MP.ProjectName
		...
		select 
		'Automation Solution' as ModuleId
		,ML.EmployeeID
		,TTT.ProjectID  
		...
		where TTT.Closeddate >= @FromDate
		and TTT.Closeddate <=  @ToDate and TTT.IsDeleted=0
		and TMT.AVMTicketType=9 group by TTT.AssignedTo,TTT.ProjectID,TTT.TicketID,ML.EmployeeID
		...
		NoOfATicketsClosed,SolutionIdentified,IncReductionMonth,EffortReductionMonth from #temTran
	...
	BEGIN CATCH
		...
		--INSERT Error                                    
		EXEC AVL_InsertError '[AVL].[GetAutomationAssociateList]'
			,@ErrorMessage
			,0
	END CATCH
END
