...
CREATE PROCEDURE [AVL].[Effort_GetUseCaseDetails] 
@ProjectID  BIGINT,
...
AS 
  BEGIN 
      BEGIN TRY 
		...
		,PT.PrimaryTechnologyName AS Technology,BPM.BusinessProcessName	AS BusinessProcess,MP.BusinessProcessName AS SubBusinessProcess,SUPPORT.ServiceLevelName AS SupportLevel
	   ...
	   INNER JOIN AVL.DEBT_PRJ_HealParentChild NDC (NOLOCK) ON NDC.ProjectPatternMapID = H.ProjectPatternMapID and ndc.IsDeleted=0 and ndc.MapStatus=1
		...
		,UCST.Technology,UCST.BusinessProcess,UCST.SubBusinessProcess,
