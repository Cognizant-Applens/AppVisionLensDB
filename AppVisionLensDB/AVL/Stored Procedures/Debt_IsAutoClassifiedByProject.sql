...
CREATE proc [AVL].[Debt_IsAutoClassifiedByProject] --4
...
from AVL.MAS_ProjectDebtDetails  WHERE ProjectID = @ProjectID and IsDeleted = 0

if(@DDClassifiedDate<= getdate() AND @IsDDAutoClassified='Y' )
begin 
SET @DDDateCheck = 'Y';
...
if(@MLSignOffDate<= getdate() AND @IsAutoClassified='Y')
begin
...
BEGIN CATCH  
		...
		--INSERT Error    
		EXEC AVL_InsertError '[AVL].[Debt_IsAutoClassifiedByProject]', @ErrorMessage, @ProjectId,0
	END CATCH  
