...
-- ============================================= 
-- Author:    471742 
-- Create date: Aug 8-2019 
-- Description:   To Get the state o fproject
-- =============================================  
CREATE PROCEDURE [AVL].[ML_SaveProcessedTicketDetailsInfra] 
(@ProjectID          BIGINT, 
@TVP_lstDebtTickets [dbo].[TVP_DebtTicketsInfra] READONLY, 
...
AS 
  BEGIN 
      BEGIN TRY 
          ...
          SELECT TicketId,TicketDescription,TowerID,DebtClassificationId, 
                 AvoidableFlagId,CauseCode,ResolutionCode,ResidualDebtId 
		  ...
		  BEGIN
			...
			SET @TranslationCount = (SELECT COUNT(1) FROM 
								  [AVL].TK_TRN_Multilingual_TranslatedInfraTicketDetails M 
								  INNER JOIN [AVL].TK_TRN_InfraTicketDetail TD ON TD.TimeTickerID = M.TimeTickerID
								  INNER JOIN #DEBTTICKETS TV ON TV.TicketID = td.TicketID
								  AND TD.ProjectID = @ProjectID AND ISTICKETDESCRIPTIONUPDATED = 1 AND TD.Isdeleted = 0)
		  END
		  
          UPDATE #DEBTTICKETS  SET    TicketDescription = NULL  WHERE  TicketDescription = '' 

          UPDATE DVT 
          SET    DVT.TicketDescription = DT.TicketDescription 
          FROM   #DEBTTICKETS DT 
                 INNER JOIN AVL.ML_TRN_TicketValidationInfra DVT ON DT.TicketId = DVT.TicketID 
                            AND DVT.ProjectId = @ProjectID AND DT.TicketDescription IS NULL 
      ...
      BEGIN CATCH 
          ...
          EXEC AVL_INSERTERROR 
            '[AVL].[ML_SaveProcessedTicketDetailsInfra] ', 
            @ErrorMessage, 
            @ProjectID, 
            0 
      END CATCH 
