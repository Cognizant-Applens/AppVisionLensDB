CREATE PROCEDURE [AVL].[GetOnPremGovernanceMetrics]
AS
BEGIN
...
DECLARE @CurrentMonths INT;
SET @CurrentMonths = (SELECT DATEPART(MONTH, GETDATE()));
DECLARE @CurrentYear INT;
IF (@CurrentMonths = 1)
BEGIN
 SET @CurrentYear = (SELECT YEAR(DATEADD(YEAR,-1 ,GETDATE())));
 SET @CurrentMonth = 13
END
ELSE
BEGIN
...
SET @ENDDATE=  DATEADD(yy, DATEDIFF(yy, -1, GETDATE()) -1, -1)  + '23:59:59'
END
ELSE
BEGIN
...
ROLLBACK TRAN
	 --INSERT Error 
	EXEC AVL_InsertError '[AVL].[GetOnPremGovernanceMetrics]', @ErrorMessage ,0
