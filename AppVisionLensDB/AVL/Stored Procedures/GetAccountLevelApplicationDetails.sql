...
CREATE PROCEDURE [AVL].[GetAccountLevelApplicationDetails]
...
If(@EsaAccountID is not null and @EsaAccountID !='')
Begin 

SET @AccountID=(select top 1 CustomerID from [AVL].[Customer](NOLOCK) where [ESA_AccountID]= @EsaAccountID and  IsDeleted=0) 
...
WHERE AD.IsActive=1 and C.ESA_AccountID=@EsaAccountID
...
BEGIN CATCH  
		...
		--INSERT Error    
		EXEC AVL_InsertError '[AVL].[GetAccountLevelApplicationDetails]', @ErrorMessage,@UserID,@AccountID
