...
CREATE PROCEDURE [RLE].[GetRoleDetailsWithProject]  
(  
...
BEGIN  
   ...
   BEGIN --Selecting AssociateName and Email  
    SELECT @AssociateName = EmployeeName,@Email = EmployeeEmail   
    FROM AVL.MAS_LoginMaster (NOLOCK)  
    WHERE IsDeleted = 0 AND EmployeeID = CONVERT(CHAR(11),@AssociateId)  
   END  
