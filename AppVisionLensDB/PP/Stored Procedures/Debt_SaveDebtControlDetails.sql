...
CREATE PROCEDURE [PP].[Debt_SaveDebtControlDetails]		 
(
...
SET 
DebtControlDate=@DebtControldate,
DDThresholdCount= @DDThresholdCount,
GracePeriod=@GracePeriod,
DebtControlMethod=ISNULL(@DebtControlMethod,DebtControlMethod), 
OptionalAttributeType = @OptionalAttributeType
,IncidentHistoricDate = case  when @IsRequireHistoricIncidents = '1' then @AHEffectiveDate else NULL END
,IsRequireHistoricIncidents = case  when @IsRequireHistoricIncidents = '1' then 1 else 0 end,
IsRequireKticket = @IsReqiureKticket 
...
-- added newly begin
	...
	DECLARE @IsMLinfra VARCHAR(50) = (SELECT IsAutoClassifiedInfra FROM AVL.MAS_ProjectDebtDetails (NOLOCK) WHERE ProjectID = @projectid AND IsDeleted = 0)
	...
	IF((@IsML != NULL) OR (@IsML != '') OR @IsDD <> NULL OR @IsDD <> '' OR @IsMLinfra <> NULL OR @IsMLinfra <> '')
	BEGIN
	print '1'
	...
	IF EXISTS (SELECT TileProgressPercentage FROM PP.ProjectProfilingTileProgress(NOLOCK) WHERE ProjectID = @ProjectID AND TileID=10  AND ISdeleted = 0)
	BEGIN
		UPDATE PP.ProjectProfilingTileProgress SET TileProgressPercentage = @Percentage,ModifiedBy = @EmployeeID , ModifiedDateTime = GETDATE()
		WHERE projectID = @ProjectID AND TileID=10 AND ISdeleted = 0
		select @Percentage = TileProgressPercentage FROM PP.ProjectProfilingTileProgress(NOLOCK) WHERE ProjectID = @ProjectID AND  TileID=10  AND ISdeleted = 0
		IF(@Percentage = 100)
		BEGIN
	...
	IF NOT EXISTS (SELECT top 1 CompletionPercentage FROM AVL.PRJ_ConfigurationProgress (NOLOCK) WHERE ProjectID = @ProjectID AND ScreenID=5  AND ISdeleted = 0)
	BEGIN
		INSERT INTO AVL.PRJ_ConfigurationProgress VALUES(@CustomerID,@ProjectID,5,NULL,100,0,@EmployeeID,GETDATE(),NULL,NULL,NULL,NULL)
	END
...
BEGIN CATCH  
		...
		ROLLBACK TRAN
		--INSERT Error    
		EXEC AVL_InsertError '[PP].[Debt_SaveDDDebtControlDetails] ', @ErrorMessage, @ProjectID,@CustomerID
