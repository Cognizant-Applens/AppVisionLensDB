...
CREATE Procedure [PP].[GetSupportTypeID]
(
@ProjectID INT
)
AS
BEGIN
		...
		BEGIN		
				IF EXISTS (SELECT top 1 1 FROM PP.ProjectAttributeValues (NOLOCK)
						JOIN (
						SELECT count(AttributeValueID) as cnt FROM PP.ProjectAttributeValues (NOLOCK)
				...
				BEGIN 
						SET @IsApp=1
				END 
				ELSE IF EXISTS (SELECT top 1 1 FROM PP.ProjectAttributeValues (NOLOCK)
						JOIN (
						SELECT count(AttributeValueID) as cnt FROM PP.ProjectAttributeValues (NOLOCK)
						WHERE ProjectID=@ProjectID and AttributeID=1  
