...
CREATE PROCEDURE [PP].[Debt_SaveIdentificationDetails]   
(  
@IsML VARCHAR(50),  
@MLDate VARCHAR(50),  
@IsDD VARCHAR(50),  
...
IF EXISTS(SELECT TOP 1 BlendedRate FROM AVL.Debt_BlendedRateCardDetails(NOLOCK) WHERE projectid = @ProjectID AND  IsApporInfra = @SupportLoop)  
 ...
 BEGIN  
  select @CompletionPercentage = TileProgressPercentage FROM PP.ProjectProfilingTileProgress(NOLOCK) WHERE ProjectID = @ProjectID AND TileID=10  AND ISdeleted = 0  
  IF(@CompletionPercentage <> 100)  
  BEGIN  
   UPDATE PP.ProjectProfilingTileProgress SET TileProgressPercentage = @Percentage,ModifiedBy = @UserID , ModifiedDateTime = GETDATE()  
   WHERE projectID = @ProjectID AND TileID=10 AND ISdeleted = 0  
  END  
 END  
 ELSE  
 BEGIN  
  INSERT INTO PP.ProjectProfilingTileProgress VALUES(@ProjectID,10,@Percentage,0,@UserID,GETDATE(),NULL,NULL)  
 END  
 ...
 END  
  -- ML MULTI Algo configuration for 'Single Algorithm Auto select'
  IF(@IsML='on' OR @IsMLinfra='on' )
  BEGIN
  IF NOT EXISTS (Select ProjectID from PP.MLMultiAlgoConfiguration Where ProjectID = @ProjectID)
  BEGIN
  INSERT INTO PP.MLMultiAlgoConfiguration
  VALUES (
  @ProjectID,
  (Select Top 1 AttributevalueId From MAS.PPAttributeValues where Isdeleted=0 and Attributeid = (Select AttributeId from mas.ppattributes where attributename ='Algorithms')),
  1,
  0,
  @UserID,
  GETDATE(),
