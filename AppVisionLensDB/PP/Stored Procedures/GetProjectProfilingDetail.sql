...
CREATE PROCEDURE [PP].[GetProjectProfilingDetail]  --44006,'ScopeOfWork'                          
...
BEGIN                                               
 ...
 BEGIN TRY                                             
  ...
  IF(@IScognizant =1)                     
  BEGIN                  
  Select @IsMainspringBUCount = count(PRD.RegistrationId) from MS.ProjectRegistrationDetails  PRD (NOLOCK)                                     
  INNER JOIN AVL.MAS_ProjectMaster (NOLOCK) PM ON PM.EsaProjectID = PRD.EsaProjectId                                      
  INNER JOIN MAS.PPAttributeValues (NOLOCK) AV1 ON AV1.AttributeValueName = PRD.ProjectOwningUnit                                      
  INNER JOIN MAS.PPAttributes (NOLOCK) A1  ON A1.AttributeID = AV1.AttributeID AND A1.AttributeName = 'MainspringPOU'                                      
  INNER JOIN MAS.PPAttributeValues (NOLOCK) AV2 ON AV2.AttributeValueName = PRD.Unit                                      
  INNER JOIN MAS.PPAttributes (NOLOCK) A2 ON A2.AttributeID = AV2.AttributeID AND A2.AttributeName = 'OPLProjectOwningUnit'                                      
  WHERE PM.projectid = @ProjectID AND PRD.IsDeleted = 0 AND PRD.Type='Predominant'AND AV1.IsDeleted = 0  AND AV2.IsDeleted = 0 AND PRD.TypeOfProject in ('Project','Group Project')                                      
                                       
  Select top 1 @MSUnit =ltrim(rtrim(PRD.Unit)) from MS.ProjectScopeDetails (NOLOCK) as PSD                                      
  INNER JOIN MS.ProjectRegistrationDetails (NOLOCK) PRD ON PRD.RegistrationId = PSD.RegistrationId                                      
  INNER JOIN AVL.MAS_ProjectMaster (NOLOCK) PM ON PM.EsaProjectID = PRD.EsaProjectId                                        
...
if(@IScognizant=1 AND  @IsMainspringBUCount > 0)                                            
                                              
  BEGIN                                       
                                         
     Select TOP 1  @OwningBUId= AV1.AttributeValueID from                                       
     MS.ProjectRegistrationDetails (NOLOCK) PRD                                      
  INNER JOIN AVL.MAS_ProjectMaster (NOLOCK) PM ON PM.EsaProjectID = PRD.EsaProjectId                                      
  INNER JOIN MAS.PPAttributeValues (NOLOCK) AV1 ON AV1.AttributeValueName = PRD.Unit                                      
  INNER JOIN MAS.PPAttributes (NOLOCK) A1 ON A1.AttributeID = AV1.AttributeID AND A1.AttributeName = 'OPLProjectOwningUnit'                                      
  WHERE PM.projectid = @ProjectID AND PRD.IsDeleted = 0 AND PRD.Type='Predominant' AND PRD.TypeOfProject in ('Project','Group Project')                                      
        AND AV1.isdeleted=0   AND A1.Isdeleted=0                                      
                                      
  Select Top 1 @MainspringArchType = AV.AttributeValueID from MS.ProjectScopeDetails (NOLOCK) PSD                                      
  INNER JOIN MS.ProjectRegistrationDetails (NOLOCK) PRD ON PRD.RegistrationId = PSD.RegistrationId                                      
  INNER JOIN AVL.MAS_ProjectMaster (NOLOCK) PM ON PM.EsaProjectID = PRD.EsaProjectId                                      
  INNER JOIN MAS.PPAttributeValues (NOLOCK) AV ON AV.AttributeValueName = PSD.Archetype                                      
  INNER JOIN MAS.PPAttributes (NOLOCK) A ON A.AttributeID = AV.AttributeID AND A.AttributeName = 'TypeofProject'                                      
  WHERE PM.projectid = @ProjectID AND PRD.IsDeleted = 0 AND PRD.Type='Predominant' AND PRD.TypeOfProject in ('Project','Group Project')                                      
  AND AV.isdeleted=0   AND A.Isdeleted=0                                      
              
  Select top 1 @ProjectType = CASE WHEN PDD.IsTransitionInScope = 'Y' Then 'Transition'                  
                                   WHEN PDD.IsTransitionInScope = 'N' Then 'Steady State'                   
                                   Else 'Not Applicable' END                  
  FROM AVL.MAS_ProjectMaster  PM  (NOLOCK)                                                      
  INNER JOIN MS.ProjectRegistrationDetails PRD  (NOLOCK) ON PRD.EsaProjectID = PM.EsaProjectID                                             
  INNER JOIN MS.ProjectDemographicDetails PDD  (NOLOCK) ON PRD.RegistrationId = PDD.RegistrationId                                                                   
  INNER JOIN MAS.PPAttributeValues (NOLOCK) AV1 ON AV1.AttributeValueName = PRD.ProjectOwningUnit                                            
  INNER JOIN MAS.PPAttributes (NOLOCK) A1 ON A1.AttributeID = AV1.AttributeID AND A1.AttributeName = 'MainspringPOU'                                            
  INNER JOIN MAS.PPAttributeValues AV2 (NOLOCK) ON AV2.AttributeValueName = PRD.Unit                                            
  INNER JOIN MAS.PPAttributes A2  (NOLOCK) ON A2.AttributeID = AV2.AttributeID AND A2.AttributeName = 'OPLProjectOwningUnit'                                            
  WHERE PM.projectid = @ProjectID AND PRD.IsDeleted = 0 AND AV1.IsDeleted = 0 AND PRD.Type='Predominant' AND PRD.TypeOfProject in ('Project','Group Project')                 
              
Set @AdditionalArchetype = REPLACE((select                
   distinct                
    stuff((              
        select ',' + PV.AttributeValueName              
        from PP.ProjectAttributevalues PAV  (NOLOCK)             
  JOIN MAS.PPattributevalues PV (NOLOCK) ON PV.AttributeValueId = PAV.AttributeValueid              
  JOIN MAS.PPAttributes PA (NOLOCK) ON PA.AttributeID = PV.AttributeID AND PAV.AttributeID = PA.AttributeId And PA.AttributeName='AdditionalArchetype'              
  where PAV.Projectid = @ProjectID and PAV.Isdeleted=0              
        order by PV.AttributeValueName              
        for xml path('')              
    ),1,1,'') as userlist              
from PP.ProjectAttributevalues PAV  (NOLOCK)             
  JOIN MAS.PPattributevalues PV (NOLOCK) ON PV.AttributeValueId = PAV.AttributeValueid              
  ...
  where PAV.Projectid = @ProjectID and PAV.Isdeleted=0), '&amp;', '&')              
     ...
     SELECT AttributeValueID,AttributeValueName,AttributeID,AttributeName,ParentID,AttributeValueOrder FROM ( SELECT  DISTINCT                                         
      PPAV.AttributeValueID                                            
     ,PPAV.AttributeValueName                                            
     ,PPA.AttributeID                                             
     ,PPA.AttributeName                                            
     ,ParentID                                            
     ,AttributeValueOrder                                            
     ...
     INNER JOIN [PP].[AttributeOwningBUMapping](NOLOCK)  OW on PPAV.Attributevalueid=OW.Attributevalueid                                            
     INNER JOIN  MAS.PPAttributes(NOLOCK) PPA on PPAV.AttributeId=PPA.AttributeId                                      
     WHERE PPAV.IsDeleted = 0  AND OW.IsDeleted=0 AND PPA.IsDeleted=0  AND OW.OwningBUId=@OwningBUId                                            
   ...
   UNION ALL                                             
    SELECT  DISTINCT                                            
       ...
       ,AttributeValueOrder                                            
    FROM                                            
     #TempnoOwningUnit(NOLOCK) ) Temp                                           
     ORDER BY AttributeValueOrder,AttributeID,AttributeValueName ASC                                          
                          
 SELECT  AttributeValueID,AttributeValueName,AttributeID,AttributeName,ParentID,AttributeValueOrder FROM #MasterDropdownValues(NOLOCK) ORDER BY AttributeValueOrder,AttributeID,AttributeValueName ASC ;                      
...
BEGIN                             
    ...
    INSERT  INTO #MasterDropdownValues                                       
   SELECT  AttributeValueID,AttributeValueName,AttributeID,AttributeName,ParentID,AttributeValueOrder FROM(                                          
       SELECT   DISTINCT                                        
         PPAV.AttributeValueID                                            
        ,AttributeValueName                                            
         ,PPA.AttributeID                                            
...
END                                            
 ...
 and pav.attributevalueID in (Select AttributeValueID from #MasterDropdownValues(NOLOCK))                      
    UNION                                            
    SELECT pav.ProjectID,pav.AttributeValueID,PPAV.AttributeID,PPA.AttributeName,@IsProjectData AS SourceOfData,PAV.CreatedBy FROM mas.PPAttributeValues(NOLOCK)  PPAV                                             
    JOIN pp.ProjectAttributeValues(NOLOCK)  PAV ON PAV.AttributeValueID = ppav.AttributeValueID                                            
    INNER JOIN  MAS.PPAttributes(NOLOCK) PPA on PPAV.AttributeId=PPA.AttributeId                                            
...
CASE WHEN @IsMainspringBUCount > 0 then ISNULL(@AdditionalArchetype,'Not Available') Else ISNULL(@AdditionalArchetype,'') End as AdditionalArchetype              
 ...
 CreatedBy nvarchar(50)                        
          ...
          IF((SELECT COUNT(ID) FROM PP.ProjectAttributeValues(NOLOCK) where ProjectID = @ProjectID and AttributeID = 9)=0)                             
    BEGIN                    
 INSERT INTO #DeliveryManagement                                            
  ...
  IF (@IsMainspringBUCount > 0 AND @IScognizant =1 )                                      
  BEGIN                                      
     Select Top 1 @MainspringShortDescription = PDD.ShortDescriptionofProject from MS.ProjectDemographicDetails PDD(NOLOCK)                                      
  ...
  SET @ShortDescription = ISNULL(@ShortDescription,@MainspringShortDescription);                                      
                                                  
   IF((SELECT COUNT(ID) FROM PP.ProjectDetails(NOLOCK) WHERE ProjectID=@ProjectID AND IsDeleted=0)=0)                                            
   BEGIN                                            
     SET @ProjectCategory=(                                            
     SELECT DISTINCT TOP 1 AV.AttributeValueID FROM PP.OplEsaData A (NOLOCK)                                            
     JOIN  MAS.PPAttributeValues AV(NOLOCK) on A.ProjectCategory=AV.AttributeValueName                                             
     WHERE A.ProjectID=@ProjectID AND AV.AttributeID=8                                            
     )                                            
   END                                            
   ELSE                                            
   BEGIN                                            
 ...
 IF(@IScognizant =1) 
 BEGIN
  SELECT Top 1 @MSVertical = PRD.ProjectVertical,@MSProjectEndDate = PRD.ProjectEndDate,                                      
  @MSOwningBu = PRD.ProjectOwningUnit,@MSTechnology = PRD.Technology,@MSUnit = PRD.Unit                                      
  from MS.ProjectRegistrationDetails PRD (NOLOCK)                                     
  INNER JOIN AVL.MAS_ProjectMaster PM(NOLOCK) ON PM.EsaProjectID = PRD.EsaProjectId                                      
  WHERE PM.projectid = @ProjectID AND PRD.IsDeleted = 0 AND PRD.Type='Predominant' AND PRD.TypeOfProject in ('Project','Group Project')                                      
 END                               
                                    
  --SET @Vertical=(SELECT DISTINCT BUName FROM AVL.BusinessUnit BU                                       
  --JOIN AVL.Customer C ON BU.BUID=C.BUID                                            
  --JOIN AVL.MAS_ProjectMaster PM ON C.CustomerID=PM.CustomerID                                             
  --WHERE PM.ProjectID=@ProjectID AND PM.IsDeleted=0 AND C.IsDeleted=0 AND BU.IsDeleted=0)                                          
                                               
                                               
   SELECT DISTINCT ProjectID,AV.AttributeValueName AS Billability,ContractYear,'' AS ContractValue,                                            
   @MSProjectEndDate AS ProjectEndDate,                                      
   FTE AS TotalFTE,Onsite AS OnsiteFTE                                            
   ,Offshore AS OffshoreFTE,                                            
   @MSOwningBu AS LineofService,                                      
   @MSTechnology AS Technology,                                      
 @MSUnit AS Unit                                      
   INTO #OplEsaData                                            
   FROM pp.OplEsaData OD (NOLOCK)                                            
   JOIN MAS.PPAttributeValues AV(NOLOCK) ON AV.AttributeValueID=OD.Billability                                            
   WHERE ProjectID =@ProjectID  AND OD.IsDeleted = 0 AND AV.IsDeleted=0                                            
                                            
   IF((SELECT COUNT(ProjectID) FROM #OplEsaData(NOLOCK))>0)                                            
   BEGIN                                            
   SELECT ProjectID,Billability,ContractYear,ContractValue,ProjectEndDate,TotalFTE,OnsiteFTE                                            
   ,OffshoreFTE,@MSVertical AS Vertical,LineofService,@ShortDescription AS ShortDescription                                            
   ,@ProjectCategory AS ProjectCategory,Technology,Unit FROM #OplEsaData (NOLOCK)                                                
   END                                            
   ELSE                                            
   BEGIN                      
