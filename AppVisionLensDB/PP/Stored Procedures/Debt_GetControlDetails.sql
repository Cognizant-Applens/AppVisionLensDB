...
CREATE PROCEDURE [PP].[Debt_GetControlDetails] --7097,10337
@CustomerID BIGINT,
@ProjectID BIGINT
...
End
Else Begin
Select @SuportTypeID = case when Attributevalueid =2 then 1 when Attributevalueid =3 then 2 end from [PP].[ProjectAttributeValues]  where ProjectID=@ProjectID  and Attributeid=1 and attributevalueid in (2,3) and Isdeleted = 0
End
...
SET @IsCognizant=(SELECT TOP 1 ISNULL(IsCognizant,'0') AS IsCognizant
						 FROM AVL.Customer (NOLOCK) WHERE CustomerID=@CustomerID AND IsDeleted<>1)

SET @ThresholdCount=(SELECT TOP 1 ISNULL(ThresholdCount,0) AS ThresholdCount FROM AVL.DEBT_MAS_HealProjectThresholdMaster (NOLOCK)
						WHERE ProjectID=@ProjectID AND IsDeleted=0)

					
SET @DDThresholdCount=(SELECT TOP 1 ISNULL(DDThresholdCount,0) AS DDThresholdCount FROM AVL.MAS_ProjectDebtDetails (NOLOCK)
WHERE ProjectID=@ProjectID AND IsDeleted=0)
...
SELECT ISNULL(DebtControlFlag,'N') AS DebtControlFlag,
       ...
       ISNULL(@ThresholdCount,0) AS ThresholdCount,
	   ISNULL(@DDThresholdCount,0) AS DDThresholdCount,
       ISNULL(@GracePeriod,7) AS GracePeriod,
       DebtControlDate,
       DebtEnablementDate AS DebtEnablementDate,@SuportTypeID as SupportTypeId, OptionalAttributeType, 
...
IF @IsCognizant = '1'
	...
	SELECT TOP 1 IsDebtEnabled FROM AVL.MAS_ProjectMaster (NOLOCK)  WHERE ProjectID = @ProjectID AND IsDeleted<>1
	-- for k ticket question --  
 SELECT TOP 1  IsRequireKticket AS IsReqiureKticket FROM AVL.MAS_ProjectDebtDetails (NOLOCK)  WHERE ProjectID = @ProjectID AND IsDeleted<>1   
