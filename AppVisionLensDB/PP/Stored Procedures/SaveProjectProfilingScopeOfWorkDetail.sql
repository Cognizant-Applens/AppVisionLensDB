...
CREATE PROCEDURE [PP].[SaveProjectProfilingScopeOfWorkDetail]              
@ProjectID BIGINT,                
...
AS                 
 ...
 IF(@IsCognizant=1)           
 BEGIN  
  Select top 1 @ProjectType = CASE WHEN PDD.IsTransitionInScope = 'Y' Then 1     
  ...
  INNER JOIN MAS.PPAttributeValues AV1(NOLOCK) ON AV1.AttributeValueName = PRD.ProjectOwningUnit                                
  INNER JOIN MAS.PPAttributes A1(NOLOCK) ON A1.AttributeID = AV1.AttributeID AND A1.AttributeName = 'MainspringPOU'                                
...
--save others values            
 ...
 END TRY               
  ...
  Select @Result as Result              
        ...
        --INSERT Error                 
   ROLLBACK TRAN              
        EXEC AVL_INSERTERROR  '[PP].[SaveProjectProfilingScopeOfWorkDetail]', @ErrorMessage,  0,0               
   END CATCH               
  END
