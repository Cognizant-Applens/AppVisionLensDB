...
CREATE PROCEDURE [MS].[JobForServiceActivity_Prod]  
		...
		IF (@ccount > 0) 
			BEGIN
					...
					FROM (SELECT
						...
						AND IsDeleted = 0 AND (ISNULL(IsMigratedFromDART,0) IN(1,2))) A
		...
		--13) To update flag in original table for the data that has been disabled in mainspring  
					...
					FROM (SELECT DISTINCT
						...
						INNER JOIN [AVL].[MAS_ProjectMaster](NOLOCK) PM
							ON PM.ProjectID = PSAM.ProjectID AND ( ISNULL(PM.IsMigratedFromDART,0) IN(1,2) )) A2
				  ...
				  on a.ServiceMapID=b.ServiceMappingID)d
					...
					BEGIN
						...
						DECLARE @MaxActivityName NVARCHAR(500);
						...
						WHILE (@Countmin <= @Countmax) 
							...
							DECLARE @ActivityName NVARCHAR(500);
							...
							SET @ActivityName=(SELECT ActivityName FROM #SPMDatainsertActivity WHERE InsertCount=@Countmin);
								IF EXISTS(SELECT * FROM AVL.MAS_ActivityMaster WHERE ActivityName=@ActivityName)
									BEGIN
										SET @maxmpsactivityid=(SELECT TOP 1 ActivityID FROM AVL.MAS_ActivityMaster 
																WHERE ActivityName=@ActivityName ORDER BY ActivityID DESC)
                                        SET @MaxActivityName=(SELECT TOP 1 ActivityName from AVL.MAS_ActivityMaster
										                         WHERE ActivityID=@maxmpsactivityid ORDER BY ActivityID DESC)
									END
								ELSE
									BEGIN
										SET @maxmpsactivityid=(SELECT TOP 1 ActivityID FROM AVL.MAS_ActivityMaster 
																	WHERE ActivityName='Others' ORDER BY ActivityID DESC)
                                        SET @MaxActivityName='Others';

									END
							DECLARE @IfExists INT;
							DECLARE @ProjectID BIGINT;
							SET @ProjectID=(SELECT PROJECTID FROM #SPMDatainsertActivity WHERE InsertCount=@Countmin)
							SET @IfExists=(SELECT COUNT(*) FROM AVL.TK_MAS_ServiceActivityMapping(NOLOCK) WHERE ServiceName=@ServiceName
											AND ActivityName=@ActivityName AND ISNULL(IsDeleted,0)=0)
							IF @IfExists=0
							BEGIN
							INSERT INTO AVL.TK_MAS_ServiceActivityMapping
								...
								--Not Used
								SET @maxservicemapid = (SELECT top 1 ServiceMappingID FROM AVL.TK_MAS_ServiceActivityMapping(NOLOCK)
														WHERE ServiceName=@ServiceName AND ActivityID = @maxmpsactivityid)
							END
							--Condition to check if there is entry for the project
							DECLARE @IsProjectMappingPresent INT;
							SET @IsProjectMappingPresent=(SELECT COUNT(*) FROM AVL.TK_PRJ_ProjectServiceActivityMapping(NOLOCK) PAM
														INNER JOIN AVL.TK_MAS_ServiceActivityMapping(NOLOCK) MAS
														ON PAM.ServiceMapID=MAS.ServiceMappingID AND MAS.ServiceName=@ServiceName
														AND MAS.ActivityName=@ActivityName  WHERE PAM.ProjectID=@ProjectID 
														AND ISNULL(MAS.IsDeleted,0)=0)

							IF @IsProjectMappingPresent=0 OR @IsProjectMappingPresent=NULL
								BEGIN
									SET @maxservicemapid = (SELECT top 1 ServiceMappingID FROM AVL.TK_MAS_ServiceActivityMapping (NOLOCK)
										...
										INSERT INTO AVL.TK_PRJ_ProjectServiceActivityMapping
										SELECT
											@maxservicemapid AS ServiceMapID
											,A.ProjectID
											,A.IsDeleted
											,A.CreatedDateTime
											,A.CreatedBY
