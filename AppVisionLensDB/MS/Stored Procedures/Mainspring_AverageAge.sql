...
-- Description:	Duration between ticket creation date and reporting date of 
--unknown incident  that are not closed

--But we will calculate based on Open TST
--Calculate the duration .
--Consider tickets which are opened till the reporting 
--end date with the status other than closed and cancelled .
--For those tickets calculate  the  sum of age as (Reporting end date - Open TST)
-- =============================================

--EXEC [MS].[Mainspring_AverageAge]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_AverageAge] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_AverageAge]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_AverageAge]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'
CREATE PROCEDURE [MS].[Mainspring_AverageAge]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	SELECT SUM(DATEDIFF(DAY,TM.OpenDateTime,@EndDate)) AS AverageAge FROM 
...
--AND TM.OpenDate >= @StartDate 
--SELECT * FROM MAS.DARTStatus

--SELECT TM.PriorityMapID FROM AppVisionLensOffline_TEST.RPT.TK_TRN_TicketDetail TM
--WHERE ProjectID=10337 AND TM.DARTStatusID NOT IN(8,5)

--UPDATE AppVisionLensOffline_TEST.RPT.TK_TRN_TicketDetail
--SET DARTStatusID=2 WHERE TicketID IN(
--'AVMInternal01',
--'AVMInternal02',
--'AVMInternal03',
--'AVMInternal04')
