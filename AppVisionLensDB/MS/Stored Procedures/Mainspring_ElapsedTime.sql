...
-- Description:	To retrieve the No. of unknown incidents in the reporting period meeting the response SLA
-- =============================================

--EXEC [MS].[Mainspring_ElapsedTime]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ElapsedTime] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ElapsedTime] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_ElapsedTime]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'


CREATE PROCEDURE [MS].[Mainspring_ElapsedTime]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	SELECT ISNULL(SUM(DATEDIFF(HOUR,TM.OpenDateTime,tm.CompletedDateTime)),0) AS ElapsedTime FROM 
	...
	WHERE TM.CompletedDateTime >= @StartDate
	 AND TM.CompletedDateTime < @EndDate AND TM.DARTStatusID IN(8,9)
