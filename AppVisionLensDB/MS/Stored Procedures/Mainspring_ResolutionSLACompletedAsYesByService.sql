...
-- Description:	(No. of Unknown  Incidents in the reporting period meeting the Resolution  SLA
--Reporting period- Closed date 
--Consider the no. of unknown incidents with status as closed and closed date falling in the reporting period with Resolution SLA as Yes
--refer rpt.Ticket master alone
-- =============================================


--EXEC [MS].[Mainspring_ResolutionSLACompletedAsYesByService] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-21',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ResolutionSLACompletedAsYesByService] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-21',@Priority=2,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ResolutionSLACompletedAsYesByService] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-21',@Priority=NULL,@SupportCategory=433,@Technology=NULL
--EXEC [MS].[Mainspring_ResolutionSLACompletedAsYesByService] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-21',@Priority=1,@SupportCategory=433,@Technology='High Level'
CREATE PROCEDURE [MS].[Mainspring_ResolutionSLACompletedAsYesByService]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	SELECT COUNT(*) AS TotalNumberResolutionYes FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK) TM
	INNER JOIN AVL.TK_MAS_Service SM WITH(NOLOCK)
	ON TM.ServiceId=SM.ServiceID  AND TM.ServiceId=@ServiceID 
	AND TM.MetResolutionMapID =1
	--AND TM.MetResolution='Y'
	AND TM.DARTStatusID IN(8,9)
