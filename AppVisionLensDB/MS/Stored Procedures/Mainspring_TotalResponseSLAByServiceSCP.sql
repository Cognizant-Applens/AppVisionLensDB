...
-- Description:	(total number of Unknown incidents with Response SLA due within the reporting period)
--Reporting Period- Closed date
--No. of unknown incidents closed in that reporting period with status as closed with response SLA as Yes or No.
--Disclaimer : Don’t take NA values
-- =============================================
--EXEC [MS].[Mainspring_TotalResponseSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResponseSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResponseSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResponseSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'

CREATE PROCEDURE [MS].[Mainspring_TotalResponseSLAByServiceSCP]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	--AVMDART_Offline.RPT
	SELECT COUNT(*) AS TotalResponseSLA FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK) TM
	INNER JOIN AVL.TK_MAS_Service SM WITH(NOLOCK)
	ON TM.ServiceId=SM.ServiceID  AND TM.ServiceId=@ServiceID 
	AND TM.MetResponseSLAMapID IN(1,2)
	--AND TM.MetResponseSLA IN('Y','N') 
