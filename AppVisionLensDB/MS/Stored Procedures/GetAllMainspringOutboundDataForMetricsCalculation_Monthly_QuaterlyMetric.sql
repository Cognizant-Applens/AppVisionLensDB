...
--EXEC [MS].[GetAllMainspringOutboundDataForMetricsCalculation_Monthly_QuaterlyMetric]
CREATE PROCEDURE [MS].[GetAllMainspringOutboundDataForMetricsCalculation_Monthly_QuaterlyMetric]
...
BEGIN
	...
	FROM MS.MAP_ProjectStage_Mapping_Monthly(NOLOCK) MPS
	...
	INNER JOIN AVL.MAS_ProjectMaster(NOLOCK) PM ON MPS.ESAProjectID=PM.EsaProjectID AND ISNULL(PM.IsDeleted,0)=0
	WHERE MPS.IsDeleted=0 AND SMBM.MetricID  IN (44) AND MPS.UniqueName IS NOT NULL
