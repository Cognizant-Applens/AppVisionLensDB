...
-- Description:Number of unknown incident Tickets reference with KEDB Articles
--Reporting period: Closed date
--Consider the no. of closed tickets with status as closed and kedb available indicator as yes
-- =============================================

--EXEC [MS].[Mainspring_IncidentswithKEDBReference] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentswithKEDBReference] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentswithKEDBReference]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentswithKEDBReference]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'

CREATE PROCEDURE [MS].[Mainspring_IncidentswithKEDBReference]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	--SELECT COUNT(*) AS KEDBReference FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK)  TM
	...
	--WHERE TM.ServiceId=@ServiceID 
	--AND TM.Closeddate >= @StartDate AND TM.Closeddate <@EndDate
	--AND TM.DARTStatusID=8
	--AND TM.KEDBAvailableIndicatorMapID = 1

--SELECT * FROM AVL.TK_MAS_KEDBAvailableIndicator
