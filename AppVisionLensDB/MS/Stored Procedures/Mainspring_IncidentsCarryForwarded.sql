...
-- Description:	Total Number of Unknown Incident  carry forwarded :
--Tickets open in previous month(s) with status not in Closed and cancelled
-- =============================================

--EXEC [MS].[Mainspring_IncidentsCarryForwarded]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsCarryForwarded] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsCarryForwarded]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsCarryForwarded]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'

CREATE PROCEDURE [MS].[Mainspring_IncidentsCarryForwarded]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	SELECT COUNT(*) AS IncidentsCarryForwarded FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK) TM
	INNER JOIN AVL.TK_MAS_Service SM WITH (NOLOCK)
	ON TM.ServiceId=SM.ServiceID AND TM.ProjectId=@ProjectID AND TM.ServiceId=@ServiceID 
	...
	WHERE TM.OpenDateTime < @StartDate AND TM.DARTStatusID NOT IN (8,5,7,13)
...
--SELECT * FROM MAS.DARTStatus(NOLOCK)
