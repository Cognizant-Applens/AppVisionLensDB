...
CREATE PROCEDURE [MS].[Mainspring_InsertValueForServiceBaseMeasureByProject]
	@ProjectStageID INT,
...
BEGIN
	...
	SET @ProjectStageIDExists=(SELECT COUNT(*) FROM MS.TRN_ProjectStaging_TillDateBaseMeasure WITH(NOLOCK)
	...
	BEGIN
		IF @ProjectStageIDExists > 0
			...
			DELETE FROM MS.TRN_ProjectStaging_TillDateBaseMeasure
			WHERE ProjectStageID=@ProjectStageID
			INSERT INTO MS.TRN_ProjectStaging_TillDateBaseMeasure (ProjectStageID,UniqueName,FrequencyID,ReportPeriodID,BaseMeasureValue,UpdatedDate,JobID,MetricStartDate,MetricEndDate)
	...
	ELSE 
		BEGIN
				DELETE FROM MS.TRN_ProjectStaging_TillDateBaseMeasure_LoadFactor
				WHERE ProjectStageID=@ProjectStageID and ReportPeriodID=@ReportPeriodID AND FrequencyID=@FrequencyID
				INSERT INTO MS.TRN_ProjectStaging_TillDateBaseMeasure_LoadFactor(ProjectStageID,UniqueName,FrequencyID,ReportPeriodID,BaseMeasureValue,UpdatedDate,JobID,MetricStartDate,MetricEndDate)
				VALUES(@ProjectStageID,@UniqueName,@FrequencyID,@ReportPeriodID,@BaseMeasureValue,@UpdatedDate,@JobID,@MetricStartDate,@MetricendDate)
