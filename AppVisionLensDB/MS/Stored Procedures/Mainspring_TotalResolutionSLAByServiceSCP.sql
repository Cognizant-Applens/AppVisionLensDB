...
-- Description:	 (total number of Unknown incidents with Resolution  SLA due within the reporting period)
--Reporting Period- Closed date
--No. of unknown incidents closed in that reporting period with statusa as closed with resolution SLA as Yes or No.
--Disclaimer : Don’t take NA values
-- =============================================


--EXEC [MS].[Mainspring_TotalResolutionSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResolutionSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResolutionSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_TotalResolutionSLAByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'

CREATE PROCEDURE [MS].[Mainspring_TotalResolutionSLAByServiceSCP]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	--AVMDART_Offline.RPT
	SELECT COUNT(*) AS TotalResolutionSLA FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK) TM
	INNER JOIN AVL.TK_MAS_Service SM WITH(NOLOCK)
	ON TM.ServiceId=SM.ServiceID 
	AND TM.MetResolutionMapID IN(1,2)
	--AND TM.MetResolution IN('Y','N')
	...
	WHERE
	TM.ProjectID=@ProjectID  AND TM.ServiceId=@ServiceID 
