...
CREATE PROCEDURE [MS].[Mainspring_GetProjectSpecificDetails_Monthly]  
(
...
--FROM  MS.MetricstagingMonthlyDump_Outbound_ProjectSpecific(nolock) A
--INNER JOIN AVL.MAS_ProjectMaster(nolock) PM ON PM.EsaProjectID =A.DN_PROJECTID
--WHERE A.DN_UNIQUEKEY=@Uniquekey
SELECT DISTINCT PM.ProjectID
FROM  MS.MetricstagingMonthlyDump_Outbound_ProjectSpecific(nolock) A
INNER JOIN AVL.MAS_ProjectMaster(nolock) PM ON PM.EsaProjectID =A.DN_PROJECTID
