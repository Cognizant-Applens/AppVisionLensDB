...
-- Description:	Actual Effort of closed unknown incidents for the reporting period
--Reporting period- Closed date 
--Consider the Sum of cumulative effort for the no. of unknown incidents with status as 
--closed and closed date falling in the reporting period 
--refer rpt.Ticket master alone

-- =============================================

--EXEC [MS].[Mainspring_ActualEffortForIncidentsClosedByServiceSCPT]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ActualEffortForIncidentsClosedByServiceSCPT] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ActualEffortForIncidentsClosedByServiceSCPT]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_ActualEffortForIncidentsClosedByServiceSCPT]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'
CREATE PROCEDURE [MS].[Mainspring_ActualEffortForIncidentsClosedByServiceSCPT]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	BEGIN
			...
			--SELECT PM.ProjectID,PMAP.MainSpringProjectPriorityID,MPM.MainspringPriorityID,P.PriorityIDMapID AS PriorityID FROM MS.MAS_Priority_Master(NOLOCK) MPM
			--INNER JOIN MS.MAP_ProjectPriority_Mapping(NOLOCK) PMAP ON MPM.MainspringPriorityID =PMAP.PriorityID
			--INNER JOIN AVL.TK_MAP_PriorityMapping(NOLOCK) P ON P.MainspringProjectPriorityID=PMAP.MainSpringProjectPriorityID
			--INNER JOIN AVL.MAS_ProjectMaster(NOLOCK) PM ON  PM.EsaProjectID=PMAP.ESAProjectID
			--WHERE PM.ProjectID=@ProjectID AND MPM.MainspringPriorityID=@Priority
	...
	BEGIN
			...
			--SELECT PM.ProjectID,AA.ApplicationID,MSCMP.MainSpringProjectSUPPORTCATEGORYID,MSC.MainspringSUPPORTCATEGORYID FROM AVL.APP_MAS_ApplicationDetails AA
			--INNER JOIN  AVL.APP_MAS_Extended_ApplicationDetail ED
			--ON AA.ApplicationID=ED.ApplicationID
			--INNER JOIN AVL.APP_MAP_ApplicationProjectMapping APM ON AA.ApplicationID=APM.ApplicationID
			--INNER JOIN MS.MAP_ProjectSUPPORTCATEGORY_Mapping MSCMP ON ED.SupportCategoryID=MSCMP.SUPPORTCATEGORYID
	...
	SELECT  ISNULL(SUM(TM.EffortTilldate),0) ActualEffort 
