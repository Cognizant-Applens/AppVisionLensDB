...
-- Description:	(No. of Unknown  Incidents in the reporting period meeting the response SLA
--Reporting period- Closed date 
--Consider the no. of unknown incidents with status as closed and closed date falling in the reporting period with Response SLA as Yes
--refer rpt.Ticket master alone
-- =============================================
--EXEC [MS].[Mainspring_ResponseSLAAsYesByServiceSCP] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ResponseSLAAsYesByServiceSCP]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_ResponseSLAAsYesByServiceSCP]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_ResponseSLAAsYesByServiceSCP]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'
CREATE PROCEDURE [MS].[Mainspring_ResponseSLAAsYesByServiceSCP]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	IF (@Technology IS NOT NULL OR @Technology <> '')
	BEGIN
	...
	--AVMDART_Offline.RPT
	SELECT COUNT(*) AS ResponseSLAYes FROM AppVisionLensOffline.RPT.TK_TRN_TicketDetail(NOLOCK) TM
	INNER JOIN AVL.TK_MAS_Service SM  WITH(NOLOCK) ON TM.ServiceId=SM.ServiceID 
	AND TM.MetResponseSLAMapID = 1
	--AND  TM.MetResponseSLA='Y' 
	...
	WHERE TM.ServiceId=@ServiceID AND TM.ProjectID=@ProjectID AND 
	TM.Closeddate >= @StartDate AND 
	TM.Closeddate < @EndDate
