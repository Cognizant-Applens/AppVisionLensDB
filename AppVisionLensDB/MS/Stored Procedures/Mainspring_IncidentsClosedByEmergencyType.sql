...
-- Description:Total Number of changes released using Emergency Change Procedure
--Consider the no. of OCM tickets closed with in the reporting period with the status as closed and release type as emergency 

--Closed tickets with status; release type as emergency
-- =============================================

--EXEC [MS].[Mainspring_IncidentsClosedByEmergencyType] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsClosedByEmergencyType] @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=NULL,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsClosedByEmergencyType]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=NULL,@SupportCategory=435,@Technology=NULL
--EXEC [MS].[Mainspring_IncidentsClosedByEmergencyType]  @ProjectID=40635,@ServiceID=4,@StartDate='2018-06-01',@EndDate='2018-06-22',@Priority=1,@SupportCategory=435,@Technology='High Level'

CREATE PROCEDURE [MS].[Mainspring_IncidentsClosedByEmergencyType]
	@ProjectID BIGINT,
	@ServiceID INT,
...
BEGIN
	...
	SELECT COUNT(*) AS IncidentsClosedByEmergencyType  
	...
	WHERE TM.Closeddate >= @StartDate AND TM.Closeddate < @EndDate
	AND TM.DARTStatusID =8
	AND TM.ReleaseTypeMapID=2
	--2 for release type as emergency
	
	--select * from AVL.TK_MAS_ReleaseType
	DROP TABLE #PRIORITY
...
--SELECT * FROM MAS.AttributeFieldMAster
