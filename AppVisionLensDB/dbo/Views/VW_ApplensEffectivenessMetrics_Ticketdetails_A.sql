...
--select * from [dbo].[VW_ApplensEffectivenessMetrics_Ticketdetails_A] where esaprojectid='1000235334' and Month='2023-03-01'
CREATE view [dbo].[VW_ApplensEffectivenessMetrics_Ticketdetails_A]
AS
WITH 
[TicketDetailsBase](TicketID,ProjectID,DARTStatusID,EffortTilldate)
As
(
...
AS
(
SELECT PPD.ProjectID,HT.HealingTicketID,
...
Group by PPD.ProjectID,HT.HealingTicketID, DATEADD(MINUTE, 30, DATEADD(HOUR, 5, HT.ClosedDate)), DATEADD(MINUTE, 30, DATEADD(HOUR, 5, HT.OpenDate)), HT.CancellationDate,HT.DARTStatusID,HT.TicketType,
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Closed_TicketType_ITD='A' then TotalAHClosed_ITD end),0)),G.Monthdate
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Cancelled_TicketTypeITD='A' then TotalAHCancelled_ITD end),0)),G.Monthdate
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Closed_TicketType_ITD='A' then DormantClosed_ITD end),0)),G.Monthdate
FROM [DebtControl_Closed_ITD] DC
left join [GetLast24Months] G ON DC.AHCloseMY_ClosedITD<=G.Monthdate 
Group by ProjectID,G.Monthdate
)
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Created_TicketType='A' then TotalAHCreated_ITD end),0)),G.Monthdate
...
As
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Closed_TicketType_ITD='A' then TotalClosedIncidentReduction_ITD end),0)),G.Monthdate
FROM [DebtControl_Closed_ITD] DC
left join [GetLast24Months] G ON DC.AHCloseMY_ClosedITD<=G.Monthdate 
Group by ProjectID,G.Monthdate
),
...
As
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Closed_TicketType_ITD='A' then DormantClosedIncidentReduction_ITD end),0)),G.Monthdate
FROM [DebtControl_Closed_ITD] DC
left join [GetLast24Months] G ON DC.AHCloseMY_ClosedITD<=G.Monthdate 
Group by ProjectID,G.Monthdate
),
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Cancelled_TicketTypeITD='A' then TotalCancelledIncidentReduction_ITD end),0)),G.Monthdate
FROM [DebtControl_Cancelled_ITD] DC
left join [GetLast24Months] G ON DC.AHCancelMY_ITD<=G.Monthdate 
Group by ProjectID,G.Monthdate
...
AS
(
SELECT ProjectID,(isnull(SUM(CASE WHEN Created_TicketType='A' then TotalCreatedIncidentReduction_ITD end),0)),G.Monthdate
FROM [DebtControl_Created_ITD] DC
left join [GetLast24Months] G ON DC.AHOpenMY_ITD<=G.Monthdate 
Group by ProjectID,G.Monthdate
),
...
Proj.EsaProjectID,
GM.MONTH1 as [Month],
isnull([Created(CM)],0) as 'Created(CM)_A',
isnull([TicketsTaggedForCreated(CM)],0) as 'Tickets Tagged For Created(CM)_A',
isnull([AvgIncidentReductionForCreated(CM)],0) as 'Avg Incident Reduction For Created(CM)_A',
isnull([AvgEffortReductionForCreated(CM)],0) as 'Avg Effort Reduction For Created(CM)_A',
isnull([Tickets Tagged For Closed (CM)],0) as [Tickets Tagged For Closed (CM)_A],
isnull([Avg Incident Reduction For Closed (CM)],0)as [Avg Incident Reduction For Closed (CM)_A],
isnull([Avg Effort Reduction For Closed (CM)],0) as [Avg Effort Reduction For Closed (CM)_A],
isnull([User Closed (CM)],0) as [User Closed (CM)_A],
isnull([Tickets Tagged For User Closed (CM)],0) as [Tickets Tagged For User Closed (CM)_A],
isnull([Avg Effort Reduction For User Closed (CM)],0) as [Avg Effort Reduction For User Closed (CM)_A],
isnull([Avg Incident Reduction For User Closed (CM)],0) as [Avg Incident Reduction For User Closed (CM)_A],
isnull([Closed],0) as [Closed_A],
isnull([Cancelled (CM)],0) as [Cancelled (CM)_A],
isnull([Tickets Tagged For Cancelled (CM)],0) as [Tickets Tagged For Cancelled (CM)_A],
isnull([Avg Incident Reduction For Cancelled (CM)],0) as [Avg Incident Reduction For Cancelled (CM)_A],
isnull([Avg Effort Reduction For Cancelled (CM)],0) as [Avg Effort Reduction For Cancelled (CM)_A],
isnull(TaggedChildTickets,0) as [Tickets Tagged to_A],
isnull(ActiveASOn,0) as [Active A As On (CM)_A],
(isnull(DCl.ATicketClosure_N,0)-isnull(DOCL.DormantClosed_ITD,0)) as ATicketClosure_N,
(isnull(DCR.ACreated_ITD,0)-isnull(DCA.ACancelled_ITD,0)-isnull(DOCL.DormantClosed_ITD,0)) as ATicketClosure_D,
isnull(AutomationEffectiveness_N,0)-isnull(ADormantIncident_ITD,0) as AutomationEffectiveness_N,
(isnull(ACreated_IncidentReduction,0)-isnull(ACancelled_IncidentReduction,0)-isnull(ADormantIncident_ITD,0)) as AutomationEffectiveness_D
