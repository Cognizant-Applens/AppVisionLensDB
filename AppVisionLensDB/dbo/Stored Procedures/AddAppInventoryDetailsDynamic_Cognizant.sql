...
CREATE PROCEDURE [dbo].[AddAppInventoryDetailsDynamic_Cognizant] 
	...
	BEGIN
		...
		INSERT INTO ADM.ALMApplicationDetails(ApplicationID,IsRevenue,IsAnySIVendor,FunctionalKnowledge,
			...
			BEGIN 
				...
				--DELETE FROM ADM.AppApplicationScope WHERE ApplicationId in (SELECT ApplicationID FROM #NewAppCogDetails)
			...
			-- Geographic part 
			 ...
			 BEGIN 
				...
				--DELETE FROM ADM.AppGeographies WHERE ApplicationId in (SELECT ApplicationID FROM #NewAppCogDetails)
		...
		BEGIN 		 
				--DELETE FROM ADM.AppRegulatoryBody WHERE ApplicationId in (SELECT ApplicationID FROM #NewAppCogDetails)
...
,s.ModifiedDate,a.ApplicationID into #ALMApplicationTemp from [ADM].[AppInventoryCognizant_Upload] s join #app a on s.ApplicationName=a.ApplicationName
	...
	----------------
	DELETE FROM  pp.ApplicationQualityAttributes WHERE ApplicationId in (SELECT ApplicationID FROM #app)
	Insert into pp.ApplicationQualityAttributes (ApplicationID,NFRCaptured,IsUnitTestAutomated,TestingCoverage,IsRegressionTest,RegressionTestCoverage,IsDeleted,CreatedBy,CreatedOn)
...
BEGIN CATCH  
		...
		ROLLBACK TRAN
		--INSERT Error    
		EXEC AVL_InsertError '[dbo].[AddAppInventoryDetailsDynamic_Cognizant] ', @ErrorMessage, 0,@CustomerId
