...
CREATE PROCEDURE [dbo].[GetAppInventoryTempdetailDynamic] --14       
 @CustomerId int =NULL      
AS      
BEGIN      
  BEGIN TRY      
...
--     SC.IsDeleted=0)) AND isCognizant=1      
  ...
  or AIU.SupportWindowName NOT IN ( SELECT       
   ...
   FROM       
     ...
     WITH(NOLOCK)      
   WHERE       
     SW.IsDeleted=0)) AND AIU.isCognizant=0    
  ...
  AIU.SupportCategoryName NOT IN (SELECT       
   ...
   FROM       
     ...
     WITH(NOLOCK)      
   WHERE       
     SC.IsDeleted=0)) AND AIU.isCognizant=0     
...
--[CloudServiceProvider]       
  ...
  LOWER(LTRIM(RTRIM(OtherWindow))) IN(SELECT LOWER(LTRIM(RTRIM(ExcludedWordName))) FROM MAS.ExcludedWords) AND AIU.isCognizant=0)      
  ...
  WHERE CustomerId=@CustomerId AND SupportWindowName<>'Others' and isCognizant=0      
  ...
  AIU.ApplicationName=ACU.ApplicationName      
  where  AIU.[CustomerId] =@CustomerId and  ACU. ApplicationScope like '%Support%'          
  and (AIU.Incallwdgreen is null or AIU.Incallwdgreen='')  
  ...
  and (AIU.Incallwdgreen is null or AIU.Incallwdgreen='')  
   ...
   --UPDATE MAS.AppInventoryUpload SET IsValid='N'      
   --where [CustomerId] =@CustomerId       
   --AND (Incallwdgreen IS NULL or Incallwdgreen='' )      
      
   -- Infraallwdgreen check       
   UPDATE  AIU set IsValid = 'N' from MAS.AppInventoryUpload AS AIU inner join [ADM].[AppInventoryCognizant_Upload] ACU on       
  AIU.ApplicationName=ACU.ApplicationName      
  where  AIU.[CustomerId] =@CustomerId and  ACU. ApplicationScope like '%Support%'          
  and (AIU.Infraallwdgreen is null or AIU.Infraallwdgreen='')  
  
   --UPDATE MAS.AppInventoryUpload SET IsValid='N'      
   --where [CustomerId] =@CustomerId       
   --AND (Infraallwdgreen IS NULL or Infraallwdgreen='')      
      
   -- Incallwdamber check       
   UPDATE  AIU set IsValid = 'N' from MAS.AppInventoryUpload AS AIU inner join [ADM].[AppInventoryCognizant_Upload] ACU on       
  AIU.ApplicationName=ACU.ApplicationName      
  where  AIU.[CustomerId] =@CustomerId and  ACU. ApplicationScope like '%Support%'          
  and (AIU.Incallwdamber is null or AIU.Incallwdamber='')  
  
   --UPDATE MAS.AppInventoryUpload SET IsValid='N'      
   --where [CustomerId] =@CustomerId       
   --AND (Incallwdamber IS NULL or Incallwdamber='')      
      
   -- SInfraallwdamber check    
   UPDATE  AIU set IsValid = 'N' from MAS.AppInventoryUpload AS AIU inner join [ADM].[AppInventoryCognizant_Upload] ACU on       
  AIU.ApplicationName=ACU.ApplicationName      
  where  AIU.[CustomerId] =1 and  ACU. ApplicationScope like '%Support%'          
  and (AIU.Infraallwdamber is null or AIU.Infraallwdamber='')  
  
   --UPDATE MAS.AppInventoryUpload SET IsValid='N'      
   --where [CustomerId] =1       
   --AND (Infraallwdamber IS NULL or Infraallwdamber='')      
...
BEGIN CATCH        
  ...
  --INSERT Error          
  EXEC AVL_InsertError '[dbo].[GetAppInventoryTempdetailDynamic]  ', @ErrorMessage, 0,@CustomerId      
