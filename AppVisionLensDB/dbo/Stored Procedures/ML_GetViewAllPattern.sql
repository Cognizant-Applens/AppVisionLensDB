...
-- [dbo].[ML_GetViewAllPattern]  1097 
-- =============================================  
CREATE PROCEDURE [dbo].[ML_GetViewAllPattern] (@ProjectID BIGINT) 
AS 
  BEGIN
...
INSERT INTO #MAXIDS
	...
	INNER JOIN #TMP_FINAL t
		ON t.ApplicationID = d.ApplicationID
...
SELECT
	...
	,ISNULL(ML.ApplicationID, 0) AS ApplicationID
	,ISNULL(AM.ApplicationName, '') AS ApplicationName
	,ISNULL(ML.ApplicationTypeID, 0) AS ApplicationTypeID
	,ISNULL(AT.ApplicationTypeName, '') AS ApplicationTypeName
	,ISNULL(ML.TechnologyID, 0) AS TechnologyID
...
FROM AVL.ML_TRN_MLPatternValidation(NOLOCK) ML
...
LEFT JOIN AVL.[App_MAS_ApplicationDetails](NOLOCK) AM
	ON ML.ApplicationID = AM.ApplicationID
...
INNER JOIN [AVL].[App_MAP_ApplicationProjectMapping](NOLOCK) AP
	ON AP.ApplicationID = ML.ApplicationID
...
AND BCM.CustomerID = @CustomerID
AND AM.isactive = 1
and ML.InitialLearningID<>@LatestID
...
SELECT
	...
	,ISNULL(ML.AdditionalSubPattern, '') AS AdditionalSubPattern
	,ISNULL(ML.ISMLSignoff, 0) AS ISMLSignoff
	,ISNULL(@IsRegenerated, 0) AS IsRegenerated INTO #TEMPREG
FROM AVL.ML_TRN_MLPatternValidation(NOLOCK) ML
JOIN AVL.ML_TRN_RegeneratedApplicationdetails(NOLOCK) REG
	ON reg.InitiallearningID = ml.InitialLearningID
	AND ml.IsDeleted = 0
	AND ml.ProjectID = @ProjectID
	AND REG.ISMLSignoff = 1
LEFT JOIN AVL.[App_MAS_ApplicationDetails](NOLOCK) AM
	ON ML.ApplicationID = AM.ApplicationID
LEFT JOIN AVL.BusinessClusterMapping(NOLOCK) BCM
	ON AM.subbusinessclustermapid = BCM.businessclustermapid
