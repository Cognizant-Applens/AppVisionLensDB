...
-- Description:   SP for Initial Learning Infra and App 

-- =============================================  
CREATE PROCEDURE [dbo].[ML_GetNoiseEliminationData_Infra] (@Selection VARCHAR(200), 
                                                    @Filter    INT, 
                                                    @ProjectID INT,
													@Mode VARCHAR(50)) 
AS 
  BEGIN 
	  ...
	  BEGIN
	   IF( @Filter >= 1 ) 
            BEGIN 
                ...
                IF( @Selection = @Top ) 
                  BEGIN 
                      SELECT TOP (SELECT @Filter) ID,ProjectID,OptionalFieldNoiseWord,Frequency,IsDeleted,CreatedDate,CreatedBy
                      FROM   AVL.ML_OptionalFieldNoiseWordsInfra 
                      WHERE  ProjectID  = @ProjectID 

                      SELECT TOP (SELECT @Filter) ID,ProjectID,TicketDescNoiseWord,Frequency,IsDeleted,CreatedDate,CreatedBy 
            ...
            BEGIN 
                IF( @Selection = @Top ) 
                  BEGIN 
                      ...
                      SELECT ID,ProjectID,OptionalFieldNoiseWord,Frequency,IsDeleted,CreatedDate,CreatedBy 
                      FROM   AVL.ML_OptionalFieldNoiseWordsInfra 
                      WHERE  ProjectID = @ProjectID 

                      SELECT ID,ProjectID,TicketDescNoiseWord,Frequency,IsDeleted,CreatedDate,CreatedBy
                      FROM   AVL.ML_TicketDescNoiseWordsInfra 
                      WHERE  ProjectID = @ProjectID 
      ...
      BEGIN CATCH 
          ...
          EXEC Avl_inserterror 
            '[dbo].[ML_GetNoiseEliminationData_Infra]', 
            @ErrorMessage, 
            @ProjectID, 
            0 
      END CATCH 
