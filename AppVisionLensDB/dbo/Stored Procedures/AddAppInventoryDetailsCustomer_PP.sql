...
CREATE PROCEDURE [dbo].[AddAppInventoryDetailsCustomer_PP]  
...
select * from #existingAppDetails)new;  
  ...
  WHERE   PM.CustomerID=@CustomerId AND EA.IsActive =0;
  ...
  JOIN avl.MAS_ProjectMaster(NOLOCK) PM  
   ON PM.ProjectID=AM.ProjectID    
  WHERE   PM.CustomerID=@CustomerId  AND AM.ProjectID =@ProjectID AND EA.IsActive = 1;
...
CustomerID BIGINT  
 ...
 SET @ApplicationName=(SELECT TOP 1 ApplicationName FROM #AppInvtopTemp)  
...
join avl.APP_MAS_ApplicationDetails mas   
 ...
 join avl.BusinessClusterMapping bcm   
 on bcm.CustomerID=@CustomerId  
  ...
  SELECT ApplicationID FROM #existingAppDetails  
  EXCEPT  
  SELECT ApplicationID FROM ADM.ALMApplicationDetails)newap;  
  
  IF EXISTS (SELECT 1 FROM #newADMAppDetails)  
  BEGIN    
   ...
   JOIN avl.APP_MAS_ApplicationDetails NA on ND.ApplicationID=NA.ApplicationID  
   JOIN [ADM].[AppInventoryCognizant_Upload] b on NA.ApplicationName=b.ApplicationName  
 ...
 ---------------------------------------------------------------------  
 IF EXISTS (SELECT 1 FROM #NewAppCogDetails)  
 BEGIN  
 ...
 and b.IsHavingSubBusinesss='0'   
   ...
   BEGIN   
     ...
     delete from ADM.AppApplicationScope where ApplicationId in (select applicationid from #AppScope)  
      
    INSERT INTO ADM.AppApplicationScope(ApplicationId,ApplicationScopeId,IsDeleted,CreatedBy,CreatedDate)  
    SELECT ApplicationId,ApplicationScopeID,0,'System',GETDATE() FROM #AppScope   
    where ApplicationID not in (select ApplicationID from ADM.AppApplicationScope)  
   ...
   -- Geographic part   
    ...
    DELETE FROM ADM.AppGeographies WHERE ApplicationId in (SELECT ApplicationID FROM #AppGeographic)  
    ...
    INSERT INTO ADM.AppGeographies(ApplicationId,GeographyId,IsDeleted,CreatedBy,CreatedDate)  
     ...
     where applicationid not in(select applicationid from ADM.AppGeographies)   
  ...
  BEGIN      
    ...
    where applicationid not in(select applicationid from ADM.AppRegulatoryBody)  
  END  
...
from pp.ApplicationQualityAttributes A   
...
select * into #Newappupload from [ADM].[AppInventoryUnitTestingCognizant_Upload] where ApplicationName not in   
(SELECT ApplicationName FROM #app  A join pp.ApplicationQualityAttributes B on A.Applicationid=b.applicationid )  
...
select d.ApplicationId,b.ApplicationName,b.OtherUnitTestFramework as 'OtherUnitTestFramework',aac.UnitTestFrameworkID as 'UnitTestFrameworkID' into #tempUnitTestingData   
 ...
 where aa.ApplicationID not in (select ApplicationID from [PP].[MAP_UnitTestingFramework])  
