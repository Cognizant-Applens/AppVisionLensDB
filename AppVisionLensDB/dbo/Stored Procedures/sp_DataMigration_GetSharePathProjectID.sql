...
CREATE PROCEDURE [dbo].[sp_DataMigration_GetSharePathProjectID]
AS
BEGIN
	...
	JOIN AVL.MAS_ProjectMaster (NOLOCK) PM 
		ON TUPC.ProjectID = PM.ProjectID AND PM.IsDeleted = 0
	JOIN AVL.Customer (NOLOCK) cust 
		ON cust.CustomerID = PM.CustomerID AND cust.IsDeleted = 0
