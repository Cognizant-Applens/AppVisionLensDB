...
Create  Procedure [dbo].[Data_Quality_Report_lastprodbackup]                                       
                      
AS                      
                      
BEGIN                        
          
 --drop table [dbo].[DataQualityReports_previousmonth]      
      
 --select  * into [dbo].[DataQualityReports_previousmonth]  from  [dbo].[DataQualityReports] where month='06-2025'      
...
OR     (C.Status <> 'Onboarded' AND TRY_CONVERT(float, TotalFTE) >= 5.0)         
-- Apply condition for other statuses               
)          
...
--and A.esaprojectid='1000428251'         
  
  
Insert stepstrack values('step1')  
...
--and A.subworkcategory in ('Production support','Incident Management','Service Request','Problem Management','Change Management')               
...
-----------------------------------------------------------------------------------                    
  ...
  SELECT DISTINCT      B.PROJECTID,      ISNULL(B.ASSOCIATEID, C.ASSOCIATEID) AS AssociateID,      C.AssociateName,      B.Dept_Name,      C.Designation,     CASE          WHEN  PATINDEX('%[0-9]%', SUBSTRING(C.Grade, 2, 2)) > 0                           
  THEN TRY_CONVERT(char, SUBSTRING(C.Grade, 2, 2))          ELSE NULL                        
  END AS Grade  INTO #Activeuserslist FROM [ESA].[ProjectAssociates] (NOLOCK) B  LEFT JOIN [ESA].[Associates] (NOLOCK) C ON C.ASSOCIATEID = B.ASSOCIATEID  WHERE        PATINDEX('%[^0-9]%', SUBSTRING(C.Grade, 2, 2)) = 0      AND TRY_CONVERT(INT, SUBSTRING 
...
(C.Grade, 2, 2)) > 50 AND B.PROJECTID IN (     SELECT DISTINCT ESA_Project_ID      FROM [dbo].[Benchmark_Eligible_Project_List](NOLOCK)     WHERE [Month] BETWEEN @StartDate AND DATEADD(MINUTE, 30, DATEADD(HOUR, 5, GETDATE())) )                     
  --NONESAUSERS                      
  ...
  INTO #NonESAUsers FROM avl.mas_loginmaster (NOLOCK) B   
  JOIN avl.mas_projectmaster (NOLOCK) A  
  ON B.ProjectID = A.ProjectID WHERE B.isnonesaauthorized = '1'                  
  AND B.isdeleted = '0'                   
  AND NOT EXISTS (         SELECT * FROM #Activeuserslist D               
  ...
  SELECT D.Associateid,D.Associatename,A.Esa_project_id,  D.Dept_Name,    D.Designation,D.Grade into #allocatedassociates FROM    [dbo].[Benchmark_Eligible_Project_List](NOLOCK) A     join #Activeuserslist D ON D.PROJECTID  =A.ESA_Project_ID              
...
FROM #allocatedassociates a FULL OUTER JOIN #nonesaassociates b  ON a.Esa_project_id = b.Esa_project_id  AND a.Associateid = b.Employeeid                    
-----------------------------------------------------------------------------------------------------------                    
...
Insert stepstrack values('step2')  
...
lEFT join [AVL].mas_projectmaster(NOLOCK) C ON C.ProjectID=B.ProjectId                    
...
D.employeeid in (select AssociateID from #FilteredAssociates  WHERE PROJECTID=C.ESAPROJECTID) and                    
B.IsDeleted='0' AND C.IsDeleted='0' and A.TimesheetDate between @StartDate and @EndDate       
group by C.EsaProjectID,A.ProjectId,DATEADD(month, DATEDIFF(month, 0, A.TimesheetDate), 0)                    
order by Month desc                    
                    
                    
   Insert stepstrack values('step3')                 
...
ON ADP.ESA_Project_ID=ATD.EsaProjectID AND ADP.MONTH=ATD.Month                    
      ...
      Insert stepstrack values('step4')              
...
from #Applens_Dashboard_Project_List A  LEFT join DiscoverEDS.EDS.[TimesheetDetail_All] B                    
...
and B.Submitterid in (select AssociateID from #FilteredAssociates WHERE PROJECTID=A.ESA_Project_ID)                    
AND B.activitydescription in  ('Problem Management','Incident Management','Service Request Management','Org Change Management','Change & Release Management')                    
...
GROUP BY  ESAProjectid,Month                    
           ...
           Insert stepstrack values('step5')         
----------------------------------------------------------------------------------------------------------------                    
...
into #Volume                    
from [AVL].[TK_TRN_TicketDetail](NOLOCK) A                    
LEFT JOIN AVL.MAS_PROJECTMASTER(NOLOCK) B ON A.PROJECTID=B.PROJECTID                    
WHERE SERVICEID IN (1,3,4,7,10,11)  --AND ESAPROJECTID='1000169049'                    
and DATEADD(MINUTE,30,DATEADD(HOUR,5,A.OpenDateTime)) BETWEEN @StartDate AND @EndDate                    
...
------------------------------------------------------------------------------------------------------------                    
     Insert stepstrack values('step6')               
--Expected Volume                    
...
left join [AVL].[TK_TRN_TicketDetail](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
WHERE  DATEADD(MINUTE,30,DATEADD(HOUR,5,C.Opendatetime)) BETWEEN @StartDate AND @EndDate and C.Efforttilldate<=0 and c.DARTStatusID in (8,9)                    
...
GROUP BY B.ESAPROJECTID, format(DATEADD(month, DATEDIFF(month, 0, DATEADD(MINUTE,30,DATEADD(HOUR,5,C.Opendatetime))), 0),'MM-yyyy')                    
   ...
   Insert stepstrack values('step7')                
----------------------------------------------------------------------------------                    
...
LEFT JOIN AVL.MAS_PROJECTMASTER(NOLOCK) B ON A.ESA_PROJECT_ID=B.ESAPROJECTID                    
left join [AVL].[TK_TRN_TicketDetail](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
where DATEDIFF(day, DATEADD(MINUTE,30,DATEADD(HOUR,5,c.OpenDateTime)), DATEADD(MINUTE,30,DATEADD(HOUR,5,GETDATE()))) >30  and dartstatusid not in (5,8,9) and  c.OpenDateTime < @StartDate                     
...
LEFT JOIN AVL.MAS_PROJECTMASTER(NOLOCK) B ON A.ESA_PROJECT_ID=B.ESAPROJECTID                    
left join [AVL].[TK_TRN_TicketDetail](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
where C.Efforttilldate>100 and C.serviceid in (4,10) and  DATEADD(MINUTE,30,DATEADD(HOUR,5,C.OpenDateTime))BETWEEN @StartDate AND @EndDate --and esaprojectid='1000296355'                    
group by  B.ESAPROJECTID, format(DATEADD(month, DATEDIFF(month, 0, DATEADD(MINUTE,30,DATEADD(HOUR,5,C.OpenDateTime))), 0),'MM-yyyy')                    
...
from #Applens_Dashboard_Project_List A                    
LEFT JOIN AVL.MAS_PROJECTMASTER(NOLOCK) B ON A.ESA_PROJECT_ID=B.ESAPROJECTID                  left join [AVL].[TK_TRN_TicketDetail](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
where C.Efforttilldate>50 and C.serviceid in (1,7) and  DATEADD(MINUTE,30,DATEADD(HOUR,5,C.OpenDateTime)) BETWEEN @StartDate AND @EndDate --and esaprojectid='1000296355'                    
group by  B.ESAPROJECTID, format(DATEADD(month, DATEDIFF(month, 0, DATEADD(MINUTE,30,DATEADD(HOUR,5,C.OpenDateTime))), 0),'MM-yyyy')                    
...
LEFT JOIN AVL.MAS_PROJECTMASTER(NOLOCK) B ON A.ESA_PROJECT_ID=B.ESAPROJECTID                    
left join [AVL].[TK_TRN_TicketDetail](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
where C.Efforttilldate<0.5 and C.serviceid in (4,10) and DATEADD(MINUTE,30,DATEADD(HOUR,5, C.OpenDateTime)) BETWEEN @StartDate AND @EndDate  --and esaprojectid='1000296355'                    
group by  B.ESAPROJECTID, format(DATEADD(month, DATEDIFF(month, 0, DATEADD(MINUTE,30,DATEADD(HOUR,5,C.OpenDateTime))), 0),'MM-yyyy')                    
...
left join [AVL].[TM_PRJ_Timesheet](NOLOCK) C ON B.PROJECTID=C.PROJECTID                    
...
where E.serviceid not in (1,7) and D.Dept_name='ADM AVM ISmO' and F.employeeid in  (select AssociateID from #FilteredAssociates)                    
AND C.TimesheetDate BETWEEN @StartDate AND @EndDate                     
group by B.ESAPROJECTID, format(DATEADD(month, DATEDIFF(month, 0,  C.Timesheetdate), 0),'MM-yyyy')                     
       
    
  Insert stepstrack values('step8')  
...
lEFT join [AVL].mas_projectmaster(NOLOCK) C ON C.ProjectID=B.ProjectId                    
left join  avl.mas_loginmaster(nolock) D on D.ProjectID=C.projectid and A.Submitterid=D.UserID  --AND D.Isdeleted='0'                    
WHERE  D.employeeid in  (select AssociateID from #FilteredAssociates WHERE PROJECTID=C.ESAPROJECTID) and                    
B.IsDeleted='0' AND C.IsDeleted='0' and A.TimesheetDate BETWEEN @StartDate AND @EndDate                     
group by C.EsaProjectID,A.ProjectId,DATEADD(month, DATEDIFF(month, 0, A.TimesheetDate), 0)                    
order by Month desc                    
...
from #Applens_Dashboard_Project_List A  LEFT join DiscoverEDS.EDS.[TimesheetDetail_All]  B                    
ON A.ESA_Project_ID=B.esaprojectid AND A.Month=format(DATEADD(month, DATEDIFF(month, 0, B.TimesheetsubmissionDate), 0),'MM-yyyy')                    
WHERE B.TimesheetsubmissionDate BETWEEN @Startdate AND @EndDate  
AND B.Submitterid in  (select AssociateID from #FilteredAssociates WHERE PROJECTID=A.ESA_Project_ID)                    
...
[CreatedDate] Datetime,[CreatedBy] NVARCHAR (50),                     
   ...
   Insert stepstrack values('step9')                 
...
delete A from  [dbo].[DataQualityReports] A where exists(select distinct Month from #DataQualityReports B WHERE A.MONTH=B.MONTH)                    
   ...
   Insert stepstrack values('step10')                 
...
delete [dbo].[DataQualityReports] where month='06-2025' and PRACTICE_AREA_PBI='IOT' and ESA_PROJECT_ID IN ( '1000423428', '1000439796', '1000447800', '1000450016', '1000450391', '1000451360')    
...
SELECT @MailBody_NoData =  [dbo].[fn_FmtEmailBody_Message](@ErrorMessage)                          
