...
-- [dbo].[ML_SaveInitialLearningState]  
-- =============================================  
CREATE PROC [dbo].[ML_SaveInitialLearningState] @ProjectID        INT, 
...
SELECT TOP 1
	[IsMLSentOrReceived] AS MLStatus
	...
	,IsSDTicket
	,IsDartTicket
	,ISNULL(IT.IsRegenerated, 0) AS IsRegenerated
	,NULL AS FromDate
	,NULL AS Todate
FROM AVL.ML_PRJ_INITIALLEARNINGSTATE IT
WHERE IT.ProjectID = @ProjectID
AND IT.ID IS NOT NULL
AND it.IsDeleted = 0
END ELSE IF (@LatestID > 0) BEGIN
UPDATE AVL.ML_PRJ_INITIALLEARNINGSTATE
SET	[StartDate] = @StartDate
	,[EndDate] = @EndDate
	,[IsMLSentOrReceived] = 'Sent'
...
LEFT JOIN AVL.ML_TRN_REGENERATEDAPPLICATIONDETAILS Reg
	ON Reg.InitialLearningID = it.ID
	AND reg.IsDeleted = 0
	AND Reg.ProjectID = it.ProjectID
	AND Reg.[IsDeleted] = 0
WHERE it.ProjectID = @ProjectID
AND IT.ID IS NOT NULL
AND it.IsDeleted = 0
AND IT.ID = @LatestID
END
END ELSE IF (@choice = 'NoiseEliminationUpd') BEGIN
...
AND ID IS NOT NULL
AND IsDeleted = 0
END ELSE IF (@LatestID > 0) UPDATE AVL.ML_PRJ_INITIALLEARNINGSTATE
SET	[StartDate] = @StartDate
	,[EndDate] = @EndDate
	,IsNoiseEliminationSentorReceived = 'Sent'
	,ModifiedBy = @UserID
...
AND it.IsDeleted = 0
AND IT.ID = @LatestID
END ELSE IF (@choice = 'SaveML') BEGIN
...
IF EXISTS (SELECT
		ProjectID
	FROM AVL.ML_PRJ_INITIALLEARNINGSTATE
	WHERE ProjectID = @ProjectID) BEGIN
SET @mlstatus = (SELECT TOP 1
		ISNULL([IsMLSentOrReceived],IsSamplingSentOrReceived)
	FROM AVL.ML_PRJ_INITIALLEARNINGSTATE
	WHERE ProjectID = @ProjectID
	AND IsDeleted = 0
	ORDER BY id DESC)

IF (@mlstatus = 'Received') BEGIN

DECLARE @IsRegenerated BIT = (SELECT TOP 1
...
INSERT INTO AVL.ML_PRJ_INITIALLEARNINGSTATE ([ProjectID],
[StartDate],
[EndDate],
[IsSDTicket],
[IsDartTicket],
[CreatedBy],
[CreatedDate],
[IsDeleted], IsRegenerated)
	VALUES (@ProjectID, @StartDate, @EndDate, @IsSMTicket, @IsDARTTicket, @UserID, GETDATE(), 0, @IsRegenerated)
...
SET	IsMLSignOff = NULL
	,MLSignOffDate = NULL
	,MLSignOffUserId = NULL
WHERE ProjectID = @ProjectID
AND IsDeleted = 0
AND @IsRegenerated = 0
...
SET	[StartDate] = @StartDate
	,[EndDate] = @EndDate
	,[IsSDTicket] = @IsSMTicket
	,[IsDartTicket] = @IsDARTTicket
	,ModifiedBy = @UserID
	,ModifiedDate = GETDATE()
...
[CreatedDate],
[IsDeleted])
	VALUES (@ProjectID, @StartDate, @EndDate, @IsSMTicket, @IsDARTTicket, @UserID, GETDATE(), 0)
...
IF EXISTS (SELECT
	...
	WHERE ProjectID = @ProjectID) BEGIN
UPDATE AVL.ML_MAP_OPTIONALPROJMAPPING
SET OptionalFieldID =
						CASE
...
AND IsActive = 1
END ELSE BEGIN
INSERT INTO AVL.ML_MAP_OPTIONALPROJMAPPING (ProjectId,
OptionalFieldID,
IsActive,
CreatedBy,
CreatedDate)
	SELECT
		...
		,GETDATE()
END
END ELSE IF (@choice = 'SamplingUpdation') BEGIN
...
--INSERT Error     
EXEC AVL_INSERTERROR	'[dbo].[ML_SaveInitialLearningState] '
						,@ErrorMessage
						,@ProjectID
						,0
END CATCH
