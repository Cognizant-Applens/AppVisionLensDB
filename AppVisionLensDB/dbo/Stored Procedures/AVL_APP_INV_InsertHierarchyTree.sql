...
--EXEC [dbo].[AVL_APP_INV_InsertHierarchyTree] 37
CREATE PROCEDURE [dbo].[AVL_APP_INV_InsertHierarchyTree]
...
SELECT * INTO #TreeNodes FROM TreeNodes
...
INSERT INTO
	@Hierarchy1
SELECT 
	DISTINCT Title, 'H1-' + CONVERT(VARCHAR(10),ID), NULL,UserName,ID
FROM 
	#TreeNodes 
CROSS APPLY
...
INSERT INTO
	@Hierarchy2
SELECT 
	DISTINCT Title, 'H2-' + CONVERT(VARCHAR(10),ID), H1.HierarchyValue, NULL, NULL,UserName,ID
FROM 
	#TreeNodes
INNER JOIN
...
INSERT INTO
	@Hierarchy3
SELECT 
	DISTINCT Title, 'H3-' + CONVERT(VARCHAR(10),ID), H2.HierarchyValue, NULL, NULL,UserName,ID
FROM 
	#TreeNodes
INNER JOIN
...
INSERT INTO
	@Hierarchy4
SELECT 
	DISTINCT Title, 'H4-' + CONVERT(VARCHAR(10),ID), H3.HierarchyValue, NULL, NULL,UserName,ID
FROM 
	#TreeNodes
INNER JOIN
...
INSERT INTO
	@Hierarchy5
SELECT 
	DISTINCT Title, 'H5-' + CONVERT(VARCHAR(10),ID), H4.HierarchyValue, NULL, NULL,UserName,ID
FROM 
	#TreeNodes
INNER JOIN
...
--	SELECT 
--	DISTINCT Title, 'H6-' + CONVERT(VARCHAR(10),ID), H5.HierarchyValue, NULL, NULL,UserName,ID
--FROM 
--	#TreeNodes
--INNER JOIN
...
FROM 
	AVL.BusinessCluster 
WHERE 
	CustomerID = @CustomerID AND ParentBusinessClusterID IS NULL
...
FROM 
	AVL.BusinessCluster 
WHERE 
	ParentBusinessClusterID = @Hierarchy1ClusterID
...
FROM 
	AVL.BusinessCluster 
WHERE 
	ParentBusinessClusterID = @Hierarchy2ClusterID
...
FROM 
	AVL.BusinessCluster 
WHERE 
	ParentBusinessClusterID = @Hierarchy3ClusterID
...
FROM 
	AVL.BusinessCluster 
WHERE 
	ParentBusinessClusterID = @Hierarchy4ClusterID
...
FROM 
	AVL.BusinessCluster 
WHERE 
	ParentBusinessClusterID = @Hierarchy5ClusterID
...
UPDATE
	H2
SET
	H2.BusinessClusterMapID = BCM.BusinessClusterMapID
FROM
	@Hierarchy2 H2
INNER JOIN
	@Hierarchy1 H1 ON H1.HierarchyValue = H2.HierarchyParentName 
INNER JOIN
	AVL.BusinessClusterMapping BCM ON H1.ClusterID = BCM.BusinessClusterID AND BCM.CustomerID = @CustomerID
	AND H1.HierarchyValue != BCM.BusinessClusterBaseName AND H1.TreeID = CONVERT(NVARCHAR(200), BCM.BusinessClusterMapID)

UPDATE
	H2
SET
	H2.BusinessClusterMapID = CASE WHEN H1.TreeID LIKE '_%' THEN BCM.BusinessClusterMapID ELSE CONVERT(BIGINT, H1.TreeID) END
FROM
	@Hierarchy2 H2
INNER JOIN
	@Hierarchy1 H1 ON H1.HierarchyValue = H2.HierarchyParentName 
...
UPDATE
	H3
SET
	H3.BusinessClusterMapID = BCM.BusinessClusterMapID
FROM
	@Hierarchy3 H3
INNER JOIN
	@Hierarchy2 H2 ON H2.HierarchyValue = H3.HierarchyParentName 
INNER JOIN
	AVL.BusinessClusterMapping BCM ON H2.ClusterID = BCM.BusinessClusterID AND BCM.CustomerID = @CustomerID
	AND H2.HierarchyValue != BCM.BusinessClusterBaseName AND H2.TreeID = CONVERT(NVARCHAR(200), BCM.BusinessClusterMapID)
	
UPDATE
	H3
SET
	H3.BusinessClusterMapID = CASE WHEN H2.TreeID LIKE '_%' THEN BCM.BusinessClusterMapID ELSE CONVERT(BIGINT, H2.TreeID) END
FROM
	@Hierarchy3 H3
INNER JOIN
	@Hierarchy2 H2 ON H2.HierarchyValue = H3.HierarchyParentName 
...
IF EXISTS(SELECT HierarchyValue FROM @Hierarchy4 WHERE HierarchyValue IS NOT NULL)
	BEGIN	
		...
		UPDATE
			H4
		SET
			H4.BusinessClusterMapID = BCM.BusinessClusterMapID
		FROM
			@Hierarchy4 H4
		INNER JOIN
			@Hierarchy3 H3 ON H3.HierarchyValue = H4.HierarchyParentName 
		INNER JOIN
			AVL.BusinessClusterMapping BCM ON H3.ClusterID = BCM.BusinessClusterID AND BCM.CustomerID = @CustomerID
			AND H3.HierarchyValue != BCM.BusinessClusterBaseName AND H3.TreeID = CONVERT(NVARCHAR(200), BCM.BusinessClusterMapID)
			
		UPDATE
			H4
		SET
			H4.BusinessClusterMapID = CASE WHEN H3.TreeID LIKE '_%' THEN BCM.BusinessClusterMapID ELSE CONVERT(BIGINT, H3.TreeID) END
		FROM
			@Hierarchy4 H4
		INNER JOIN
			@Hierarchy3 H3 ON H3.HierarchyValue = H4.HierarchyParentName 
		...
		IF EXISTS(SELECT HierarchyValue FROM @Hierarchy5 WHERE HierarchyValue IS NOT NULL)
			BEGIN	
				...
				UPDATE
					H5
				SET
					H5.BusinessClusterMapID = BCM.BusinessClusterMapID
				FROM
					@Hierarchy5 H5
				INNER JOIN
					@Hierarchy4 H4 ON H4.HierarchyValue = H5.HierarchyParentName 
				INNER JOIN
					AVL.BusinessClusterMapping BCM ON H4.ClusterID = BCM.BusinessClusterID AND BCM.CustomerID = @CustomerID
					AND H4.HierarchyValue != BCM.BusinessClusterBaseName AND H4.TreeID = CONVERT(NVARCHAR(200), BCM.BusinessClusterMapID)
					
				UPDATE
					H5
				SET
					H5.BusinessClusterMapID = CASE WHEN H4.TreeID LIKE '_%' THEN BCM.BusinessClusterMapID ELSE CONVERT(BIGINT, H4.TreeID) END
				FROM
					@Hierarchy5 H5
				INNER JOIN
					@Hierarchy4 H4 ON H4.HierarchyValue = H5.HierarchyParentName 
...
WHEN NOT MATCHED THEN
	...
	VALUES (H5.HierarchyValue, H5.ClusterID, H5.BusinessClusterMapID, 					
				...
				UPDATE
					H6
				SET
					H6.BusinessClusterMapID = BCM.BusinessClusterMapID
				FROM
					@Hierarchy6 H6
				INNER JOIN
					@Hierarchy5 H5 ON H5.HierarchyValue = H6.HierarchyParentName 
				INNER JOIN
					AVL.BusinessClusterMapping BCM ON H5.ClusterID = BCM.BusinessClusterID AND BCM.CustomerID = @CustomerID
					AND H5.HierarchyValue != BCM.BusinessClusterBaseName AND H5.TreeID = CONVERT(NVARCHAR(200), BCM.BusinessClusterMapID)
					
				UPDATE
					H6
				SET
					H6.BusinessClusterMapID = CASE WHEN H5.TreeID LIKE '_%' THEN BCM.BusinessClusterMapID ELSE CONVERT(BIGINT, H5.TreeID) END
				FROM
					@Hierarchy6 H6
				INNER JOIN
					@Hierarchy5 H5 ON H5.HierarchyValue = H6.HierarchyParentName 
...
SELECT 1
	...
	SET @CognizantId= (select UserName from #TreeNodes)
	...
	VALUES(@CustomerId,1,50,0,@CognizantId,GETDATE())
	END
...
BEGIN CATCH
	...
	DROP TABLE TreeNodes
		...
		--INSERT Error    
		EXEC AVL_InsertError '[dbo].[AVL_APP_INV_InsertHierarchyTree]', @ErrorMessage, '0', @CustomerID 
END CATCH
