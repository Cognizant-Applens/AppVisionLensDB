...
CREATE proc [dbo].[ML_MLGetPatternOccurencetestInfra]--'Infra check acc','0','0','0',240,'','20117',14027,15236
...
ELSE
BEGIN
SELECT ITEM INTO #tempidTower FROM split(@TowerID,',')
...
SELECT distinct
			...
			FROM AVL.ML_TRN_MLPatternValidationInfra MV

			JOIN AVL.InfraTowerDetailsTransaction TD  ON MV.TowerID=TD.InfraTowerTransactionID AND TD.IsDeleted=0

			INNER JOIN #tempidTower t ON t.item = MV.TowerID

			
			 join AVL.InfraTowerProjectMapping TPM ON  MV.ProjectID=TPM.ProjectID AND MV.TowerID=TPM.TowerID AND TPM.IsDeleted=0 AND TPM.IsEnabled=1

			LEFT JOIN [AVL].DEBT_MAS_DebtClassificationInfra AFM ON MV.MLDebtClassificationID= AFM.[DebtClassificationID]
...
DECLARE @ErrorMessage VARCHAR(MAX);
		...
		ROLLBACK TRAN
		--INSERT Error    
		EXEC AVL_InsertError '[dbo].[ML_MLGetPatternOccurence] ', @ErrorMessage, @projectID,@AppID
