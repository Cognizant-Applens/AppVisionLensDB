CREATE PROCEDURE  [dbo].[Tk_Ticketupload_sharepath]    
(      
  ...
  @TvpTicketUpload as [TVP_TicketUpload] readonly ,    
...
--#region Create Create and insert into ImportTicketDumpDetails    
 ...
 [IsPartiallyAutomated] INT NULL,    
  ...
  INSERT into #ImportTicketDumpDetails ([Ticket ID],[Ticket Type],[Open Date],Priority,Status,Application,[External Login ID],Assignee,[Modified Date Time],[Reopen Date],[Ticket Description],    
            ...
            [Flex Field (2)],[Flex Field (3)],[Flex Field (4)],Category,Type,EmployeeID,EmployeeName,ProjectID,TicketLocation,Reviewer,[Resolution Method],    
            TicketUploadTrackID,IsTicketSummaryModified,IsTicketDescriptionModified,    
...
REPLACE(REPLACE(IsPartiallyAutomated,'<',''),'>','')AS IsPartiallyAutomated,    
 0 AS IsGracePeriodMet,REPLACE(REPLACE([TicketDescriptionBasePattern] ,'<',''),'>','')AS TicketDescriptionBasePattern,    
 ...
 From @TvpTicketUpload     
  ...
  SELECT @IsPartiallyMapped=ProjectColumn       
    ...
    WHERE  projectid = @projectid       
     ...
     UPDATE  dbo.TicketUpload set IsPartiallyAutomated=null where projectID = @projectid  and (IsPartiallyAutomated='0' or IsPartiallyAutomated='')     
         
    if @IsPartiallyMapped =''    
      BEGIN    
      UPDATE  #ImportTicketDumpDetails set IsPartiallyAutomated=null where projectID = @projectid     
      END    
    ELSE    
      BEGIN    
         UPDATE  #ImportTicketDumpDetails set IsPartiallyAutomated=null where projectID = @projectid AND (LTRIM(rtrim( IsPartiallyAutomated))='' OR IsPartiallyAutomated='0')    
      UPDATE  #ImportTicketDumpDetails set IsPartiallyAutomated='InValid' where projectID = @projectid  and IsPartiallyAutomated is not null AND IsPartiallyAutomated not in (select MetSLAName from AVL.TK_MAS_MetSLACondition)     
      UPDATE TU SET TU.IsPartiallyAutomated=SLA.MetSLAId FROM #ImportTicketDumpDetails TU  INNER JOIN AVL.TK_MAS_MetSLACondition SLA ON LTRIM(RTRIM(TU.IsPartiallyAutomated)) =SLA.MetSLAName AND projectID = @projectid    
      END    
    ...
    SELECT TOP 1 @GracePeriodValue=GracePeriod FROM [AVL].[MAS_ProjectDebtDetails] WHERE ProjectID=@projectid AND IsDeleted<>1    
      
     UPDATE D    
               SET D.IsGracePeriodMet=1 FROM #ImportTicketDumpDetails D     
         ...
         ( (Convert(date, (DATEADD(DAY,@GracePeriodValue,D.[Close Date])))< Convert(date, GETDATE()) AND B.TicketStatus_ID=8 AND D.[Close Date] IS NOT NULL AND             D.[Close Date] != '') OR     
  ...
  UPDATE DumpTic SET DumpTic.IsGracePeriodMet=1 FROM #ImportTicketDumpDetails DumpTic      
     INNER JOIN AVL.DEBT_PRJ_InfraHealParentChild INFRACH ON INFRACH.DARTTicketID =DumpTic.[Ticket ID]    
  ...
  AND HPPM.ProjectID = @projectid AND INFRACH.MapStatus=1 AND INFRACH.IsDeleted<>1    
   ...
   BEGIN      
   SELECT TOP 1  @CogID=EmployeeID FROM #ImportTicketDumpDetails      
   WHERE ProjectID=@projectid;      
   END       
         
      update #ImportTicketDumpDetails  set TicketUploadTrackID =  @TicketUploadTrackID where ProjectID = @projectid    
...
WHERE  projectid = @projectid       
   ...
   IF ( @Flag = 'N' )       
       ...
       x.IsBOT = 0         
            ...
            FROM   #ImportTicketDumpDetails x       
...
BEGIN       
 ...
 X2.[kedb available indicator] = X3.kedbavailableindicatorname       
   ...
   SELECT DISTINCT @CustomerID=CustomerID FROM AVL.MAS_ProjectMaster PM WHERE ProjectID=@projectid AND IsDeleted=0;    
   SELECT DISTINCT @IsCognizant=IsCognizant FROM AVL.Customer WHERE CustomerID=@CustomerID AND IsDeleted=0;    
      UPDATE X2     
...
FROM #ImportTicketDumpDetails TT    
LEFT JOIN AVL.TK_TRN_TicketDetail TD ON TT.ProjectID=TD.ProjectID    
AND TT.[Ticket ID]=TD.TicketID WHERE TD.TicketID IS NULL AND TT.SupportType = 1    
    
UPDATE TT SET TT.IsGracePeriodMet =0    
FROM #ImportTicketDumpDetails TT    
LEFT JOIN AVL.TK_TRN_InfraTicketDetail TD ON TT.ProjectID=TD.ProjectID    
AND TT.[Ticket ID]=TD.TicketID WHERE TD.TicketID IS NULL AND TT.SupportType = 2    
    
-- New code    
UPDATE TT SET TT.IsGracePeriodMet =0    
FROM #ImportTicketDumpDetails TT    
INNER JOIN AVL.TK_TRN_TicketDetail TD ON TT.ProjectID=TD.ProjectID and TT.[Ticket ID]=TD.TicketID    
...
WHERE  projectid = @projectid        
    ...
    IF @SourceID <> ''       
      ...
      BEGIN       
          ...
          SET @FailCnt = (SELECT Count([ticket id])       
                          ...
                          FROM   #ImportTicketDumpDetails_NullValue       
 ...
 END      
  ...
  SELECT @GracePeriodMetTickets=COUNT([Ticket ID]) FROM #ImportTicketDumpDetails_Nullvalue WHERE ISNULL(Remarks,'')=''  AND ISNULL([RemarksForGracePeriod],'')<>''     
...
END      
 ...
 DELETE #ImportTicketDumpDetails       
      ...
      WHERE  projectid = @projectid       
             ...
             AND ( ([External Login ID] IS NOT NULL and [External Login ID]!='' and userid IS NULL)     
 ...
 UPDATE TEMP SET TEMP.[Ticket Description] = null     
  FROM #ImportTicketDumpDetails AS TEMP    
 ...
 [ITSM Effort]                      ,    
 [Assignment Group]                 ,    
 [Assignment Group ID]              ,   [UploadedBy]                       ,    
 [UploadedDate]                     ,    
 ...
 IsPartiallyAutomated,    
 IsGracePeriodMet    
 from #ImportTicketDumpDetails WHERE ISNULL(SupportType,1) = 2 and IsNull(IsBOT,0) = 0     
...
FROM  #ImportTicketDumpDetails ITD LEFT JOIN AVL.TK_TRN_TicketDetail TD WITH (NOLOCK) ON TD.TicketID=ITD.[Ticket ID]     
...
FROM  #ImportTicketDumpDetails_Infra ITD LEFT JOIN AVL.TK_TRN_InfraTicketDetail TD WITH (NOLOCK) ON TD.TicketID=ITD.[Ticket ID]     
AND TD.ProjectID=@projectid AND TD.IsDeleted=0;    
 ...
 MERGE [AVL].[TK_TRN_BOTTicketDetail] TD     
    ...
    WHEN matched THEN     
     ...
     TD.TowerID = ISNULL(T1.[TowerID],TD.[TowerID] ),    
                 ...
                 TD.[ticketdescription] =CASE WHEN RTRIM(LTRIM(t1.[ticket description]))!= '' AND RTRIM(LTRIM(t1.[ticket description])) IS NOT NULL THEN  T1.[ticket description] ELSE TD.[ticketdescription] END,    
                 TD.[IsDeleted] = ISNULL(T1.IsDeleted,  0),                      TD.[causecodemapid] = ISNULL(T1.[causecodeid],  TD.[causecodemapid]),    
                 TD.[debtclassificationmapid] = ISNULL(T1.[debtclassificationid], TD.[debtclassificationmapid]),    
                 TD.[residualdebtmapid] = ISNULL(T1.[residualdebtid], TD.[residualdebtmapid]),    
                 TD.[resolutioncodemapid] = ISNULL(T1.[resolutionid],   TD.[resolutioncodemapid]),    
    --TD.[ResolutionMethodMapID]= ,     
...
Null, --'InscopeOutscope',     
 ...
 /**************************GRACE PERIOD UNDO UPDATE For Infra Tickets***********************/    
  ...
  [Resolution Method]=(CASE WHEN  (Debt.OptionalAttributeType=2 OR Debt.OptionalAttributeType=3) AND @ResolutionRemarksInfra=1 THEN TD.ResolutionRemarks ELSE [Resolution Method] END)    
       ...
       FROM #ImportTicketDumpDetails_Infra T INNER JOIN [AVL].[TK_TRN_InfraTicketDetail](NOLOCK) TD    
       ON T.[Ticket ID] = TD.TicketID  AND TD.ProjectID  = @ProjectID AND TD.IsDeleted<>1    
      LEFT JOIN [AVL].MAS_ProjectDebtDetails (NOLOCK) AS Debt ON Debt.ProjectID=T.ProjectID AND Debt.IsDeleted<>1    
       WHERE T.ProjectID=@ProjectID AND  T.IsGracePeriodMet=1 and td.DARTStatusID in ('8','9')     
...
Update  [AVL].[TK_TRN_InfraTicketDetail]  set IsAttributeUpdated='0' where (DARTStatusID='8' or DARTStatusID='9') and (Closeddate is null or DebtClassificationMapID is null or CauseCodeMapID is null or ResolutionCodeMapID is null)     
and projectid=@ProjectID    
    
 --ErrorData to c# code.Result set 1    
        SELECT [Ticket ID],Remarks    
 +ISNULL([RemarksForGracePeriod],'') AS Remarks FROM #ImportTicketDumpDetails_Nullvalue    
    
 --Result set 2     
 DECLARE @count int     
 set @count = (select count([Ticket ID]) from #ImportTicketDumpDetails_Nullvalue WHERE IsGracePeriodMet<>1)    
 ...
 SELECT [Ticket ID],ITDD.ProjectID FROM #ImportTicketDumpDetails ITDD    
  ...
  AND CC.IsDeleted = 0    
  JOIN AVL.DEBT_MAP_ResolutionCode RC    
 ...
 DECLARE @SupportTypeApp int;     
 DECLARE @SupportTypeInfra int;        
      
 set @SupportTypeApp =(SELECT TOP 1 SupportType FROM #ImportTicketDumpDetails (NOLOCK) WHERE PROJECTID=@ProjectID)      
 --IF (@SupportType IS NULL)      
 --BEGIN        
  set @SupportTypeInfra =(SELECT TOP 1 SupportType FROM #ImportTicketDumpDetails_Infra (NOLOCK) WHERE PROJECTID=@ProjectID)      
 --END      
        
 IF (@SupportTypeApp=1 AND @AppAlgorithmKey='AL001')       
 BEGIN       
 ...
 IF (@SupportTypeInfra=2 AND @InfraAlgorithmKey='AL001')      
 BEGIN      
  ...
  BEGIN    
    ...
    from #ImportTicketDumpDetails_Infra       
   where CauseCodeID is NOT NULL AND ResolutionID IS NOT NULL AND (DARTStatusId = 8 OR DARTStatusId = 9) AND      
   DebtClassificationId is NULL and AvoidableFlagID is NULL and ResidualDebtID is NULL      
  END    
  ELSE IF (@AutoClassificationType=2)    
...
END              
  ...
  BEGIN      
 --tickets for auto Classification insert mode--    App           
...
FROM #ImportTicketDumpDetails WHERE (DARTStatusId = 8 OR DARTStatusId = 9) AND DebtClassificationId is NULL and AvoidableFlagID is NULL and ResidualDebtID is NULL END              
...
SET DCM.DebtClassficationMode=3,dcm.SystemDebtclassification=IM.DebtClassificationId,    
 ...
 DCM.ResolutionCodeID=TD.ResolutionCodeMapID,DCM.SourceForPattern=(CASE WHEN @mode='SharePath' THEN 3 ELSE 2 END),    
 DCM.ModifiedDate=GETDATE(),DCM.ModifiedBy=@CogID, DCM.SystemCauseCodeID = NULL, DCM.SystemResolutionCodeID = NULL      
FROM #ImportTicketDumpDetails IM      
JOIN AVL.TK_TRN_TicketDetail TD ON TD.ProjectID=@ProjectID AND IM.ProjectID=TD.ProjectID AND TD.TicketID=IM.[Ticket ID]    
JOIN AVL.Debt_MAS_ProjectDataDictionary PDD ON PDD.ProjectID=@ProjectID AND IM.ApplicationID=PDD.ApplicationID AND IM.CauseCodeID=PDD.CauseCodeID    
 ...
 AND TD.IsDeleted=0     
JOIN AVL.TRN_DebtClassificationModeDetails DCM ON DCM.TimeTickerID=TD.TimeTickerID     
 AND DCM.Isdeleted=0 AND (((TD.DebtClassificationMapID IS NOT NULL AND TD.AvoidableFlag IS NOT NULL     
 ...
 AND PDD.CauseCodeID IS NOT NULL AND PDD.ResolutionCodeID IS NOT NULL    
 AND PDD.DebtClassificationID IS NOT NULL AND PDD.AvoidableFlagID IS NOT NULL    
 AND PDD.ResidualDebtID IS NOT NULL AND DCM.DebtClassficationMode NOT IN (1,2)    
...
--------------------------------    
 ...
 JOIN #ImportTicketDumpDetails IM ON IM.[Ticket ID]=ticket.TicketID AND IM.ProjectID=ticket.ProjectID 
...
LEFT JOIN AVL.TRN_InfraDebtClassificationModeDetails DCM     
 ON  DCM.TimeTickerID = TD.TimeTickerID AND TD.ProjectID = @ProjectID AND TD.IsDeleted = 0      
WHERE DCM.ID IS NULL AND IM.CauseCodeID IS NOT NULL AND IM.ResolutionID IS NOT NULL AND IM.CauseCodeID IS NOT NULL    AND IM.ResolutionID IS NOT NULL AND IM.DebtClassificationId IS NOT NULL AND IM.AvoidableFlagID IS NOT NULL    
  AND IM.ResidualDebtID IS NOT NULL AND TD.CauseCodeMapID IS NOT NULL AND TD.ResolutionCodeMapID IS NOT NULL      
  AND TD.DebtClassificationMapID IS NOT NULL AND TD.AvoidableFlag  IS NOT NULL AND TD.ResidualDebtMapID IS NOT NULL    
...
SET DCM.DebtClassficationMode=5,dcm.SystemDebtclassification=NULL,DCM.SystemAvoidableFlag=NULL,      
 ...
 DCM.ResolutionCodeID=TD.ResolutionCodeMapID,DCM.SourceForPattern=(CASE WHEN @mode='SharePath' THEN 3 ELSE 2 END),    
 DCM.ModifiedDate=GETDATE(),DCM.ModifiedBy=@CogID , DCM.SystemCauseCodeID = NULL, DCM.SystemResolutionCodeID = NULL    
FROM #ImportTicketDumpDetails_Infra IM      
JOIN  AVL.TK_TRN_InfraTicketDetail TD     
 ON TD.ProjectID=@ProjectID AND IM.ProjectID=TD.ProjectID AND TD.TicketID=IM.[Ticket ID] AND TD.IsDeleted=0     
JOIN AVL.Debt_MAS_ProjectDataDictionary PDD ON PDD.ProjectID=@ProjectID AND IM.ApplicationID <> PDD.ApplicationID    
 AND IM.CauseCodeID <> PDD.ResolutionCodeID AND IM.DebtClassificationId <> PDD.DebtClassificationID    
 AND IM.AvoidableFlagID <> PDD.AvoidableFlagID AND PDD.IsDeleted=0    
JOIN AVL.TRN_InfraDebtClassificationModeDetails DCM ON DCM.TimeTickerID=TD.TimeTickerID     
 AND DCM.Isdeleted=0 AND (((TD.DebtClassificationMapID IS NOT NULL AND TD.AvoidableFlag IS NOT NULL     
 AND TD.ResidualDebtMapID IS NOT NULL )) OR ((    
 (DCM.CauseCodeID<>TD.CauseCodeMapID OR DCM.ResolutionCodeID<>TD.ResolutionCodeMapID)     
 AND IM.DebtClassificationId IS NOT NULL AND IM.AvoidableFlagID IS NOT NULL AND IM.ResidualDebtID IS NOT NULL)))     
...
SET DCM.DebtClassficationMode=3,dcm.SystemDebtclassification=IM.DebtClassificationId,    
 ...
 DCM.ModifiedDate=GETDATE(),DCM.ModifiedBy=@CogID ,DCM.SystemCauseCodeID = NULL, DCM.SystemResolutionCodeID = NULL     
FROM  #ImportTicketDumpDetails_Infra IM      
JOIN AVL.TK_TRN_InfraTicketDetail TD ON TD.ProjectID=@ProjectID AND IM.ProjectID=TD.ProjectID AND TD.TicketID=IM.[Ticket ID]     
JOIN AVL.Debt_MAS_ProjectDataDictionary PDD ON PDD.ProjectID=@ProjectID AND IM.ApplicationID=PDD.ApplicationID AND IM.CauseCodeID=PDD.CauseCodeID    
 AND IM.ResolutionID=PDD.ResolutionCodeID AND IM.DebtClassificationId=PDD.DebtClassificationID    
 AND IM.AvoidableFlagID=PDD.AvoidableFlagID AND IM.ResidualDebtID=PDD.ResidualDebtID AND PDD.IsDeleted=0    
...
update ticket set         
 ...
 AND ticket.ProjectID=@ProjectID AND ticket.IsDeleted=0       
 JOIN #ImportTicketDumpDetails_Infra IM ON IM.[Ticket ID]=ticket.TicketID AND IM.ProjectID=ticket.ProjectID 
 AND ISNULL(ticket.DebtClassificationMode,0)<>5	--Restrict Override the Manual tickets   
 ...
 FROM AVL.ErrorLogCorrectionTickets  T     
...
FROM #MultilingualTbl2 ITD JOIN AVL.TK_TRN_TicketDetail TD WITH (NOLOCK) ON TD.TicketID=ITD.[Ticket ID]     
AND TD.ProjectID=@projectid AND TD.IsDeleted=0;    
    
UPDATE ITD SET ITD.TimeTickerID=TD.TimeTickerID    
FROM #MultilingualTblInfra ITD JOIN AVL.TK_TRN_InfraTicketDetail TD WITH (NOLOCK) ON TD.TicketID=ITD.[Ticket ID]     
AND TD.ProjectID=@projectid AND TD.IsDeleted=0;    
...
SOURCE.IsCategoryModified,SOURCE.IsTypeModified,0,@CogID,GETDATE(),2);    
...
--SELECT * from [AVL].[TK_TRN_Multilingual_TranslatedTicketDetails]    
 ...
 WHERE TicketUploadTrackID=@TicketUploadTrackID    
   --INSERT Error          
   EXEC AVL_InsertError '[dbo].[Tk_Ticketupload_Sharepath]', @ErrorMessage, @CogID, @projectID      
