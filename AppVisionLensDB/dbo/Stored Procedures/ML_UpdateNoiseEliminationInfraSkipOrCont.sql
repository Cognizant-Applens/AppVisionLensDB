...
-- =============================================  
CREATE PROCEDURE [dbo].[ML_UpdateNoiseEliminationInfraSkipOrCont] --318,'627384' 
  @ProjectID BIGINT, 
  @UserID    NVARCHAR(500) 
AS 
  BEGIN 
      BEGIN TRY 
          ...
          DELETE FROM AVL.ML_OptionalFieldNoiseWordsInfra 
          WHERE  ProjectID = @ProjectID 

          DELETE FROM AVL.ML_TicketDescNoiseWordsInfra 
          WHERE  ProjectID = @ProjectID 
      ...
      BEGIN CATCH 
          ...
          EXEC AVL_INSERTERROR 
            'ML_UpdateNoiseEliminationInfraSkipOrCont', 
            @ErrorMessage, 
            @ProjectID, 
            0 
      END CATCH 
