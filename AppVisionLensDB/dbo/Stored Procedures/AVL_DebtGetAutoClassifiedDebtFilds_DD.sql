...
***************************************************************************/
CREATE PROCEDURE [dbo].[AVL_DebtGetAutoClassifiedDebtFilds_DD] 
...
SET @ISTicketTypeApplicable=(SELECT COUNT(TicketTypeMappingID)  FROM AVL.TK_MAP_TicketTypeMapping (NOLOCK) WHERE ProjectID=@ProjectID AND DebtConsidered='Y' AND TicketTypeMappingID=@TicketTypeID )
...
where DIC.CauseCodeID = @Causecode and DIC.ResolutionCodeID = @Resolutioncode AND ISNULL(DIC.IsDeleted,0)=0
...
UserDebtClassificationFlag,UserAvoidableFlag,UserResidualDebtFlag,SourceForPattern,IsDeleted,CreatedBy,CreatedDate,DebtClassficationMode) 
 ...
 UPDATE AVL.TRN_DebtClassificationModeDetails SET SystemDebtclassification=@DebtClassificationflag,SystemAvoidableFlag=@AvoidableFlag,
 SystemResidualDebtFlag=@ResidualFlag,UserDebtClassificationFlag=NULL,UserAvoidableFlag=NULL,UserResidualDebtFlag=NULL,
 SystemCauseCodeID= null,SystemResolutionCodeID= null,
 SourceForPattern=1,DebtClassficationMode=3,ModifiedDate=GETDATE(),ModifiedBy=@UserID,CauseCodeID=@Causecode,ResolutionCodeID=@Resolutioncode
