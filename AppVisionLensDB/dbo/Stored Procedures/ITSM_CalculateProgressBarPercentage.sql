...
CREATE proc [dbo].[ITSM_CalculateProgressBarPercentage]
@ProjectID INT,
@CustomerID INT = NULL
AS
BEGIN
BEGIN TRY
	...
	BEGIN
       ...
       ELSE 
		...
		SELECT CAST (@MainIndex AS INT) AS ITSMScreenID, 
			   @IsCognizant AS IsCognizant,
			   CASE WHEN (@finalPerc) >100.00 THEN 100.00 ELSE @finalPerc END AS StatusProgress,
			   ISNULL(@IsSeverity, 0) AS 'IsSeverity', 
...
BEGIN CATCH  
		...
		--INSERT Error    
		EXEC AVL_InsertError '[dbo].[ITSM_CalculateProgressBarPercentage] ', @ErrorMessage, @ProjectID,@CustomerID
